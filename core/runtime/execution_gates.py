...
class RuntimeRiskGate:
    ...
    def __init__(self, *, max_quantity: float | None = None, allowed_symbols: set[str] | None = None,
                 allowed_sides: set[str] | None = None, max_notional: float | None = None):
        self._max_quantity = max_quantity
        self._allowed_symbols = allowed_symbols
        self._allowed_sides = allowed_sides or {"buy", "sell"}
        self._max_notional = max_notional

    def approve(self, signal: Signal, order: OrderRequest, market: dict | None = None) -> ExecutionApproval:
        reasons = []
        market = market or {}
        ...
        if self._max_notional is not None:
            price = market.get("price") or market.get("last") or market.get("ask") or market.get("bid") or 0
            if order.quantity * float(price) > self._max_notional:
                reasons.append(f"notional_limit_exceeded({order.quantity * float(price):.2f}>{self._max_notional:.2f})")
...
class RuntimeGovernanceGate:
    ...
    def __init__(self, *, allowed_strategy_ids: set[str] | None = None, frozen_strategy_ids: set[str] | None = None,
                 allowed_venues: set[str] | None = None, system_halted: bool = False):
    ...
    def approve(self, signal: Signal, order: OrderRequest, market: dict | None = None) -> ExecutionApproval:
        ...
        if self._allowed_strategy_ids is not None and signal.strategy_id not in self._allowed_strategy_ids:
...
class RuntimeExecutionApprovalChain:
    ...
    def approve(self, signal: Signal, order: OrderRequest, market: dict | None = None) -> list[ExecutionApproval]:
        approvals = []
