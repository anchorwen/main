...
class AlphaRiskBudgetGate:
    ...
    def __init__(self, risk_budget: dict, *, deny_missing: bool = True, usage_store: AlphaBudgetUsageStore | None = None):
    ...
    def approve(self, signal: Signal, order: OrderRequest, market: dict | None = None) -> ExecutionApproval:
        ...
        if max_daily is not None and self._count(alpha_id) + 1 > int(max_daily):
            reasons.append(f"alpha_daily_order_limit_exceeded({self._count(alpha_id) + 1}>{int(max_daily)})")
    ...
    def _approval(self, signal: Signal, order: OrderRequest, reasons: list[str], constraints: dict) -> ExecutionApproval:
        ...
        return ExecutionApproval.allow(approval_id, signal, order, "alpha_risk_budget", constraints=constraints)
    ...
    def _count(self, alpha_id: str) -> int:
        return self._usage_store.get(alpha_id) if self._usage_store else self._daily_counts.get(alpha_id, 0)
    ...
    def _notional(self, order: OrderRequest, market: dict) -> float:
        price = market.get("price") or market.get("last") or market.get("ask") or market.get("bid") or order.limit_price or 0
