...
class ExecutionEventReader:
    ...
    def find_by_message_id(self, *, date_key: str, correlation_id: str, message_id: str) -> list[dict]:
        return [
            event for event in self.list_events(date_key=date_key, correlation_id=correlation_id)
            if event.get(PAYLOAD_KEY_MESSAGE_ID) == message_id
        ]

    def find_terminal_event(self, *, date_key: str, correlation_id: str, message_id: str) -> dict | None:
        terminal_types = {TERMINAL_EVENT_REJECTED, TERMINAL_EVENT_FILLED, TERMINAL_EVENT_CANCELLED, TERMINAL_EVENT_EXPIRED}
        events = self.find_by_message_id(date_key=date_key, correlation_id=correlation_id, message_id=message_id)
        for event in reversed(events):
            if event.get(PAYLOAD_KEY_EVENT_TYPE) in terminal_types:
                return event
        return None

    def find_latest_event(self, *, date_key: str, correlation_id: str, message_id: str) -> dict | None:
        events = self.find_by_message_id(date_key=date_key, correlation_id=correlation_id, message_id=message_id)
        return events[-1] if events else None

    def build_execution_timeline(self, *, date_key: str, correlation_id: str, message_id: str) -> dict:
        events = self.find_by_message_id(date_key=date_key, correlation_id=correlation_id, message_id=message_id)
        total_filled_qty = sum(
            event.get(PAYLOAD_KEY_QUANTITY, {}).get(TERMINAL_EVENT_FILLED, 0)
            for event in events
            if event.get(PAYLOAD_KEY_EVENT_TYPE) in {TERMINAL_EVENT_PARTIALLY_FILLED, TERMINAL_EVENT_FILLED}
        )
        terminal = None
        for event in reversed(events):
            if event.get(PAYLOAD_KEY_EVENT_TYPE) in {TERMINAL_EVENT_REJECTED, TERMINAL_EVENT_FILLED, TERMINAL_EVENT_CANCELLED, TERMINAL_EVENT_EXPIRED}:
                terminal = event
                break
        ...
        return {
            ...
            PAYLOAD_KEY_TERMINAL_EVENT_TYPE: terminal.get(PAYLOAD_KEY_EVENT_TYPE) if terminal else None,
            ...
            PAYLOAD_KEY_LATEST_EVENT_TYPE: events[-1].get(PAYLOAD_KEY_EVENT_TYPE) if events else None,
        }
