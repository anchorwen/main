...
class OperationsTimelineService:
    ...
    def record(self, event_type: str, payload: dict, *, actor: str = RELEASE_PIPELINE_DEFAULT_ACTOR) -> dict:
    ...
    def record_release_gate(self, report: dict, *, actor: str = RELEASE_PIPELINE_DEFAULT_ACTOR) -> dict:
        return self.record(TIMELINE_EVENT_RELEASE_GATE, report, actor=actor)

    def record_deployment_execution(self, result: dict, *, actor: str = RELEASE_PIPELINE_DEFAULT_ACTOR) -> dict:
        return self.record(TIMELINE_EVENT_DEPLOYMENT_EXECUTION, result, actor=actor)

    def record_rollback_drill(self, result: dict, *, actor: str = RELEASE_PIPELINE_DEFAULT_ACTOR) -> dict:
        return self.record(TIMELINE_EVENT_ROLLBACK_DRILL, result, actor=actor)

    def record_evidence_bundle(self, result: dict, *, actor: str = RELEASE_PIPELINE_DEFAULT_ACTOR) -> dict:
        return self.record(TIMELINE_EVENT_EVIDENCE_BUNDLE, result, actor=actor)

    def record_alpha_budget_governance(self, result: dict, *, actor: str = RELEASE_PIPELINE_DEFAULT_ACTOR) -> dict:
    ...
    def summarize(self) -> dict:
        ...
        by_status = {}
        for event in events:
            by_type[event[PAYLOAD_KEY_EVENT_TYPE]] = by_type.get(event[PAYLOAD_KEY_EVENT_TYPE], 0) + 1
    ...
    def _infer_status(self, payload: dict) -> str:
        if PAYLOAD_KEY_DECISION in payload:
            ...
            return TIMELINE_STATUS_PASSED if decision in {RELEASE_PIPELINE_GATE_DECISION_ALLOW, RELEASE_PIPELINE_GATE_DECISION_WARN} else TIMELINE_STATUS_FAILED
        if PAYLOAD_KEY_PASSED in payload:
            return TIMELINE_STATUS_PASSED if payload.get(PAYLOAD_KEY_PASSED) else TIMELINE_STATUS_FAILED
        if PAYLOAD_KEY_VERIFIED in payload:
            return TIMELINE_STATUS_PASSED if payload.get(PAYLOAD_KEY_VERIFIED) else TIMELINE_STATUS_FAILED
        ...
        if payload.get(PAYLOAD_KEY_SCHEMA_VERSION) == SCHEMA_ENGINE_CONFIG_RELOAD_EVENT:
            return TIMELINE_STATUS_PASSED if not payload.get(PAYLOAD_KEY_ERROR) else TIMELINE_STATUS_FAILED
        if PAYLOAD_KEY_GATE_DECISION in payload:
            return TIMELINE_STATUS_PASSED if payload.get(PAYLOAD_KEY_GATE_DECISION) in {RELEASE_PIPELINE_GATE_DECISION_ALLOW, RELEASE_PIPELINE_GATE_DECISION_WARN} else TIMELINE_STATUS_FAILED
        return payload.get(PAYLOAD_KEY_STATUS, TIMELINE_STATUS_UNKNOWN)

    def _summarize_payload(self, event_type: str, payload: dict) -> dict:
        if event_type == TIMELINE_EVENT_RELEASE_GATE:
            return {PAYLOAD_KEY_DECISION: payload.get(PAYLOAD_KEY_DECISION), PAYLOAD_KEY_STRICT: payload.get(PAYLOAD_KEY_STRICT)}
        if event_type == TIMELINE_EVENT_DEPLOYMENT_EXECUTION:
            return {PAYLOAD_KEY_STATUS: payload.get(PAYLOAD_KEY_STATUS), PAYLOAD_KEY_VERSION: payload.get(PAYLOAD_KEY_VERSION),
        ...
        if event_type == TIMELINE_EVENT_EVIDENCE_BUNDLE:
            return {PAYLOAD_KEY_LABEL: payload.get(PAYLOAD_KEY_LABEL), PAYLOAD_KEY_GATE_DECISION: payload.get(PAYLOAD_KEY_GATE_DECISION),
        ...
        if event_type == TIMELINE_EVENT_ALPHA_BUDGET_GOVERNANCE:
            return {
                ...
                PAYLOAD_KEY_MISSING_EVIDENCE_COUNT: payload.get(PAYLOAD_KEY_MISSING_EVIDENCE_COUNT, 0),
