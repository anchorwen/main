...
class ConfigValidator:
    ...
    def _register_defaults(self) -> None:
        self._rules.append(("base_dir_not_empty",
            lambda c: bool(c.base_dir)))
        self._rules.append(("max_open_positions_positive",
            lambda c: c.max_open_positions > 0))
        self._rules.append(("max_drawdown_valid",
            lambda c: 0.0 < c.max_drawdown_pct <= 100.0))
        self._rules.append(("max_notional_positive",
            lambda c: c.max_notional_exposure > 0))
        self._rules.append(("max_per_symbol_positive",
            lambda c: c.max_per_symbol > 0))
        self._rules.append(("feedback_window_positive",
            lambda c: c.feedback_window_size > 0))
        self._rules.append(("idempotency_ttl_positive",
            lambda c: c.idempotency_ttl_hours > 0))
        self._rules.append(("producer_name_set",
            lambda c: bool(c.producer_name)))
        self._rules.append(("target_name_set",
            lambda c: bool(c.target_name)))
        self._rules.append(("ops_maturity_min_score_in_range",
            lambda c: 0.0 <= float(getattr(c, "ops_maturity_min_score", 60.0)) <= 100.0))
        self._rules.append(("engine_config_poll_interval_nonnegative",
        ...
        for name, check_fn in self._rules:
            try:
                if not check_fn(config):
                    errors.append({PAYLOAD_KEY_RULE: name, PAYLOAD_KEY_STATUS: HEALTH_CHECK_STATUS_FAILED})
            except Exception as exc:
                errors.append(
                    {PAYLOAD_KEY_RULE: name, PAYLOAD_KEY_STATUS: LIFECYCLE_PHASE_STATUS_ERROR, PAYLOAD_KEY_DETAIL: str(exc)}
                )
