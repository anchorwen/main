...
class LifecycleManager:
    ...
    def startup(self, *, restore_state: bool = False, state_label: str = "latest") -> dict:
        ...
        result[PAYLOAD_KEY_PHASES].append({PAYLOAD_KEY_PHASE: LIFECYCLE_PHASE_HEALTH_CHECK, PAYLOAD_KEY_RESULT: health})

        if restore_state and self._persistence:
            restored = self._persistence.restore_governance_state(
                self._container.governance_service, label=state_label,
            )
        ...
        if self._container.audit_log:
            self._container.audit_log.log(
                event_type="lifecycle", severity="info",
                actor="lifecycle_manager",
                detail={PAYLOAD_KEY_ACTION: LIFECYCLE_AUDIT_ACTION_STARTUP, PAYLOAD_KEY_RESTORE_STATE: restore_state},
            )
    ...
    def shutdown(self, *, save_state: bool = True, state_label: str | None = None) -> dict:
        ...
        for hook in self._shutdown_hooks:
            try:
                hook()
                result[PAYLOAD_KEY_PHASES].append({PAYLOAD_KEY_PHASE: LIFECYCLE_PHASE_HOOK, PAYLOAD_KEY_STATUS: HEALTH_CHECK_STATUS_OK})
            except Exception as exc:
                result[PAYLOAD_KEY_PHASES].append(
                    {PAYLOAD_KEY_PHASE: LIFECYCLE_PHASE_HOOK, PAYLOAD_KEY_STATUS: LIFECYCLE_PHASE_STATUS_ERROR, PAYLOAD_KEY_ERROR: str(exc)}
                )

        if save_state and self._persistence:
            label = state_label or datetime.utcnow().strftime("%Y%m%d_%H%M%S")
            save_result = self._persistence.save_all(self._container, label=label)
            result[PAYLOAD_KEY_PHASES].append(
                {PAYLOAD_KEY_PHASE: LIFECYCLE_PHASE_STATE_SAVE, PAYLOAD_KEY_LABEL: label, PAYLOAD_KEY_PATHS: save_result[PAYLOAD_KEY_PATHS]}
            )
        ...
        if self._container.audit_log:
            self._container.audit_log.log(
                event_type="lifecycle", severity="info",
                actor="lifecycle_manager",
                detail={PAYLOAD_KEY_ACTION: LIFECYCLE_AUDIT_ACTION_SHUTDOWN, PAYLOAD_KEY_UPTIME_SECONDS: result[PAYLOAD_KEY_UPTIME_SECONDS]},
            )
