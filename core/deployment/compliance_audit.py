...
class ComplianceAuditService:
        ...
        report = {
            ...
            PAYLOAD_KEY_SUMMARY: {
                ...
                PAYLOAD_KEY_FAILED_COUNT: len(
                    [c for c in checks if c[PAYLOAD_KEY_LEVEL] == COMPLIANCE_LEVEL_FAIL]
                ),
                PAYLOAD_KEY_WARNING_COUNT: len(
                    [c for c in checks if c[PAYLOAD_KEY_LEVEL] == COMPLIANCE_LEVEL_WARN]
                ),
                PAYLOAD_KEY_REGISTRY_RECORD_COUNT: registry_summary.get(
                    PAYLOAD_KEY_RECORD_COUNT, 0
                ),
                PAYLOAD_KEY_TIMELINE_EVENT_COUNT: timeline_summary.get(PAYLOAD_KEY_EVENT_COUNT, 0),
                PAYLOAD_KEY_VALIDATION_MODE_COUNTS: registry_summary.get(
                    PAYLOAD_KEY_VALIDATION_MODE_COUNTS, {}
                ),
                PAYLOAD_KEY_ALPHA_BUDGET_EVIDENCE_COUNT: registry_summary.get(
                    PAYLOAD_KEY_ALPHA_BUDGET, {}
                ).get(PAYLOAD_KEY_EVIDENCE_COUNT, 0),
                PAYLOAD_KEY_ALPHA_BUDGET_WARNING_TOTAL: registry_summary.get(
                    PAYLOAD_KEY_ALPHA_BUDGET, {}
                ).get(PAYLOAD_KEY_WARNING_TOTAL, 0),
                PAYLOAD_KEY_REGISTRY_FINAL_AUDIT: registry_summary.get(PAYLOAD_KEY_FINAL_AUDIT, {}),
                PAYLOAD_KEY_REGISTRY_OPS_MATURITY: registry_summary.get(
                    PAYLOAD_KEY_OPS_MATURITY, {}
                ),
    ...
    def _build_checks(
        ...
        timeline_failed = timeline_summary.get(PAYLOAD_KEY_STATUS_COUNTS, {}).get(
        ...
        checks.append(
            self._check(
                COMPLIANCE_CHECK_OPERATIONS_TIMELINE_PRESENT,
                COMPLIANCE_LEVEL_PASS
                if timeline_summary.get(PAYLOAD_KEY_EVENT_COUNT, 0) > 0
        ...
        checks.append(
            self._check(
                ...
                COMPLIANCE_LEVEL_PASS
                if not latest
                or latest.get(PAYLOAD_KEY_SUMMARY, {}).get(PAYLOAD_KEY_EVIDENCE_VERIFIED)
                else COMPLIANCE_LEVEL_FAIL,
                {
                    PAYLOAD_KEY_LATEST_ID: latest.get(PAYLOAD_KEY_ID),
                    PAYLOAD_KEY_EVIDENCE_VERIFIED: latest.get(PAYLOAD_KEY_SUMMARY, {}).get(
                        PAYLOAD_KEY_EVIDENCE_VERIFIED
                    ),
                },
            )
        ...
        checks.append(
            self._check(
                COMPLIANCE_CHECK_ALPHA_BUDGET_EVIDENCE_REGISTERED,
                COMPLIANCE_LEVEL_PASS
                if record_count == 0 or alpha_budget.get(PAYLOAD_KEY_MISSING_EVIDENCE_COUNT, 0) == 0
                else COMPLIANCE_LEVEL_WARN,
                {
                    PAYLOAD_KEY_RECORD_COUNT: record_count,
        ...
        checks.append(
            self._check(
                COMPLIANCE_CHECK_REGISTRY_DEEP_VALIDATION_PRESENT,
                COMPLIANCE_LEVEL_PASS
                if record_count == 0 or deep_count > 0
                else COMPLIANCE_LEVEL_WARN,
                {
                    PAYLOAD_KEY_RECORD_COUNT: record_count,
        ...
        checks.append(
            self._check(
                ...
                fa_level,
                {
                    ...
                    PAYLOAD_KEY_UNKNOWN_READINESS_COUNT: fa.get(
                        PAYLOAD_KEY_UNKNOWN_READINESS_COUNT, 0
                    ),
                    PAYLOAD_KEY_MISSING_EVIDENCE_COUNT: fa.get(
                        PAYLOAD_KEY_MISSING_EVIDENCE_COUNT, 0
                    ),
                },
            )
        ...
        if record_count == 0:
            ...
            om.get(PAYLOAD_KEY_SCORE_AVG) is not None
            and float(om.get(PAYLOAD_KEY_SCORE_AVG, 0)) < min_ops
        ):
            om_level = COMPLIANCE_LEVEL_WARN
        else:
            om_level = COMPLIANCE_LEVEL_PASS
        checks.append(
            self._check(
                ...
                om_level,
                {
                    ...
                    PAYLOAD_KEY_RECORD_COUNT: record_count,
                    PAYLOAD_KEY_STRATEGY_COUNT_WITH_SCORE: om.get(
    ...
    def _check(self, name: str, level: str, detail: dict) -> dict:
        return {
            ...
            PAYLOAD_KEY_PASSED: level == COMPLIANCE_LEVEL_PASS,
        ...
        for check in checks:
            ...
            if check[PAYLOAD_KEY_NAME] == COMPLIANCE_CHECK_RELEASE_REGISTRY_PRESENT:
                recs.append(
                    {
                        ...
                        PAYLOAD_KEY_ACTION: "Register at least one release certificate",
                    }
                )
                ...
                recs.append(
                    {
                        ...
                        PAYLOAD_KEY_ACTION: "Investigate rejected or uncertified release records",
                    }
                )
                ...
                recs.append(
                    {
                        ...
                        PAYLOAD_KEY_ACTION: "Review failed operations timeline events",
                    }
                )
                ...
                recs.append(
                    {
                        PAYLOAD_KEY_PRIORITY: RECOMMENDATION_PRIORITY_CRITICAL,
                        PAYLOAD_KEY_ACTION: "Fix release readiness failures",
                    }
                )
                ...
                recs.append(
                    {
                        PAYLOAD_KEY_PRIORITY: RECOMMENDATION_PRIORITY_CRITICAL,
                        PAYLOAD_KEY_ACTION: "Resolve release gate blocking signals",
                    }
                )
                ...
                recs.append(
                    {
                        PAYLOAD_KEY_PRIORITY: RECOMMENDATION_PRIORITY_HIGH,
                        PAYLOAD_KEY_ACTION: "Restore SLO compliance",
                    }
                )
                ...
                recs.append(
                    {
                        PAYLOAD_KEY_PRIORITY: RECOMMENDATION_PRIORITY_CRITICAL,
                        PAYLOAD_KEY_ACTION: "Regenerate or verify latest release evidence bundle",
                    }
                )
                ...
                recs.append(
                    {
                        PAYLOAD_KEY_PRIORITY: RECOMMENDATION_PRIORITY_MEDIUM,
                        PAYLOAD_KEY_ACTION: (
                            "Attach Alpha budget usage evidence to all release certificates"
                        ),
                    }
                )
                ...
                recs.append(
                    {
                        PAYLOAD_KEY_PRIORITY: RECOMMENDATION_PRIORITY_MEDIUM,
                        PAYLOAD_KEY_ACTION: (
                            "Review Alpha budget usage warnings before the next release"
                        ),
                    }
                )
                ...
                recs.append(
                    {
                        PAYLOAD_KEY_PRIORITY: RECOMMENDATION_PRIORITY_MEDIUM,
                        PAYLOAD_KEY_ACTION: (
                            "Ensure every registered release has final audit evidence"
                            " and a cleared final audit"
                        ),
                    }
                )
                ...
                recs.append(
                    {
                        PAYLOAD_KEY_PRIORITY: RECOMMENDATION_PRIORITY_MEDIUM,
                        PAYLOAD_KEY_ACTION: (
                            f"Raise ops maturity score to >= {m} and register releases"
                            " with full maturity evidence"
                        ),
                    }
                )
            ...
            recs.append(
                {
                    ...
                    PAYLOAD_KEY_ACTION: "Archive compliance audit report",
                }
            )
        return recs
