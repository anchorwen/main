...
class PostmortemReportService:
    ...
    def _build_engine_config_timeline(self, timeline: list[dict]) -> dict:
        events = [e for e in timeline if e.get(PAYLOAD_KEY_EVENT_TYPE) == TIMELINE_EVENT_ENGINE_CONFIG]
        return {
    ...
    def _build_alpha_budget_analysis(self, timeline: list[dict]) -> dict:
        events = [event for event in timeline if event.get(PAYLOAD_KEY_EVENT_TYPE) == TIMELINE_EVENT_ALPHA_BUDGET_GOVERNANCE]
        warning_total = sum(event.get(PAYLOAD_KEY_SUMMARY, {}).get(PAYLOAD_KEY_WARNING_TOTAL, 0) for event in events)
        missing_evidence_count = sum(event.get(PAYLOAD_KEY_SUMMARY, {}).get(PAYLOAD_KEY_MISSING_EVIDENCE_COUNT, 0) for event in events)
        evidence_count = sum(event.get(PAYLOAD_KEY_SUMMARY, {}).get(PAYLOAD_KEY_EVIDENCE_COUNT, 0) for event in events)
        statuses = {}
        for event in events:
            statuses[event.get(PAYLOAD_KEY_STATUS)] = statuses.get(event.get(PAYLOAD_KEY_STATUS), 0) + 1
        return {
            PAYLOAD_KEY_EVENT_COUNT: len(events),
            ...
            PAYLOAD_KEY_WARNING_EVENT_COUNT: len([event for event in events if event.get(PAYLOAD_KEY_SUMMARY, {}).get(PAYLOAD_KEY_WARNING_TOTAL, 0) > 0]),
            PAYLOAD_KEY_EVENTS: [
                {
                    PAYLOAD_KEY_ID: event.get(PAYLOAD_KEY_ID),
                    PAYLOAD_KEY_TIMESTAMP: event.get(PAYLOAD_KEY_TIMESTAMP),
    ...
    def _build_impact(
        ...
        failed_events = [event for event in timeline if event.get(PAYLOAD_KEY_STATUS) == TIMELINE_STATUS_FAILED]
        alpha_budget = alpha_budget or {}
        engine_config = engine_config or {}
        return {
            PAYLOAD_KEY_FAILED_EVENT_COUNT: len(failed_events),
            PAYLOAD_KEY_FAILED_EVENT_TYPES: sorted({event.get(PAYLOAD_KEY_EVENT_TYPE) for event in failed_events}),
            PAYLOAD_KEY_RELEASE_BLOCKED: gate.get(PAYLOAD_KEY_DECISION) == RELEASE_PIPELINE_GATE_DECISION_BLOCK,
            ...
            PAYLOAD_KEY_ALPHA_BUDGET_MISSING_EVIDENCE: alpha_budget.get(PAYLOAD_KEY_MISSING_EVIDENCE_COUNT, 0),
            PAYLOAD_KEY_ENGINE_CONFIG_TIMELINE_EVENTS: engine_config.get(PAYLOAD_KEY_EVENT_COUNT, 0),
            PAYLOAD_KEY_CUSTOMER_IMPACT: POSTMORTEM_IMPACT_VALUE_UNKNOWN,
            PAYLOAD_KEY_FINANCIAL_IMPACT: POSTMORTEM_IMPACT_VALUE_UNKNOWN,
        }

    def _build_findings(self, timeline: list[dict], gate: dict, slo: dict, diagnostics: dict,
                        alpha_budget: dict | None = None) -> list[dict]:
        findings = []
        failed_events = [event for event in timeline if event.get(PAYLOAD_KEY_STATUS) == TIMELINE_STATUS_FAILED]
        if failed_events:
            findings.append({
                ...
                PAYLOAD_KEY_SUMMARY: f"{len(failed_events)} failed operational event(s) recorded",
        ...
        if gate.get(PAYLOAD_KEY_DECISION) == RELEASE_PIPELINE_GATE_DECISION_BLOCK:
            findings.append({
                ...
                PAYLOAD_KEY_EVIDENCE: gate.get(PAYLOAD_KEY_SUMMARY, {}).get(PAYLOAD_KEY_BLOCKING_SIGNALS, []),
            })
        ...
        if alpha_budget.get(PAYLOAD_KEY_MISSING_EVIDENCE_COUNT, 0) > 0:
            findings.append({
                ...
                PAYLOAD_KEY_SUMMARY: "Alpha budget governance evidence is missing from one or more release events",
        ...
        if alpha_budget.get(PAYLOAD_KEY_WARNING_TOTAL, 0) > 0:
            findings.append({
                ...
                PAYLOAD_KEY_SEVERITY: FINDING_SEVERITY_MEDIUM,
                PAYLOAD_KEY_SUMMARY: f"Alpha budget governance recorded {alpha_budget.get(PAYLOAD_KEY_WARNING_TOTAL, 0)} warning(s)",
                PAYLOAD_KEY_EVIDENCE: alpha_budget,
            })
        audit_summary = diagnostics.get(PAYLOAD_KEY_AUDIT_SUMMARY, {}) if diagnostics.get(PAYLOAD_KEY_AVAILABLE) else {}
        error_count = (audit_summary.get(PAYLOAD_KEY_SEVERITY_COUNTS) or {}).get(PAYLOAD_KEY_ERROR, 0)
        if error_count:
            findings.append({
    ...
    def _build_actions(self, findings: list[dict]) -> list[dict]:
        ...
        for finding in findings:
            if finding[PAYLOAD_KEY_ID] == FINDING_ID_TIMELINE_FAILURES:
                actions.append({PAYLOAD_KEY_OWNER: ACTION_OWNER_OPERATIONS, PAYLOAD_KEY_PRIORITY: RECOMMENDATION_PRIORITY_HIGH,
                                PAYLOAD_KEY_ACTION: ACTION_TEXT_TIMELINE_FAILURES})
            elif finding[PAYLOAD_KEY_ID] == FINDING_ID_RELEASE_GATE_BLOCKED:
                actions.append({PAYLOAD_KEY_OWNER: ACTION_OWNER_RELEASE, PAYLOAD_KEY_PRIORITY: RECOMMENDATION_PRIORITY_CRITICAL,
                                PAYLOAD_KEY_ACTION: ACTION_TEXT_RELEASE_GATE_BLOCKED})
            elif finding[PAYLOAD_KEY_ID] == FINDING_ID_SLO_BREACH:
                actions.append({PAYLOAD_KEY_OWNER: ACTION_OWNER_RELIABILITY, PAYLOAD_KEY_PRIORITY: RECOMMENDATION_PRIORITY_HIGH,
                                PAYLOAD_KEY_ACTION: ACTION_TEXT_SLO_BREACH})
            elif finding[PAYLOAD_KEY_ID] == FINDING_ID_AUDIT_ERRORS:
                actions.append({PAYLOAD_KEY_OWNER: ACTION_OWNER_ENGINEERING, PAYLOAD_KEY_PRIORITY: RECOMMENDATION_PRIORITY_MEDIUM,
                                PAYLOAD_KEY_ACTION: ACTION_TEXT_AUDIT_ERRORS})
            elif finding[PAYLOAD_KEY_ID] == FINDING_ID_ALPHA_BUDGET_EVIDENCE_MISSING:
                actions.append({PAYLOAD_KEY_OWNER: ACTION_OWNER_RELEASE, PAYLOAD_KEY_PRIORITY: RECOMMENDATION_PRIORITY_MEDIUM,
                ...
                actions.append({PAYLOAD_KEY_OWNER: ACTION_OWNER_RISK, PAYLOAD_KEY_PRIORITY: RECOMMENDATION_PRIORITY_MEDIUM,
                                PAYLOAD_KEY_ACTION: ACTION_TEXT_ALPHA_BUDGET_WARNINGS_PRESENT})
        if not actions:
            actions.append({PAYLOAD_KEY_OWNER: ACTION_OWNER_OPERATIONS, PAYLOAD_KEY_PRIORITY: RECOMMENDATION_PRIORITY_LOW,
