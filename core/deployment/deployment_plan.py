...
class DeploymentPlanService:
    ...
    def build_plan(
        ...
        *,
        ...
        executable = gate[PAYLOAD_KEY_DECISION] in {RELEASE_PIPELINE_GATE_DECISION_ALLOW, RELEASE_PIPELINE_GATE_DECISION_WARN}
        return {
            PAYLOAD_KEY_SCHEMA_VERSION: SCHEMA_DEPLOYMENT_PLAN,
            ...
            PAYLOAD_KEY_VALIDATION_MODE: validation_mode,
            PAYLOAD_KEY_STATUS: DEPLOYMENT_PLAN_STATUS_READY if executable else RELEASE_PIPELINE_STATUS_BLOCKED,
            ...
            PAYLOAD_KEY_COMMANDS: self._build_commands(version, strategy, validation_mode=validation_mode),
        }
    ...
    def _build_phases(self, strategy: str) -> list[dict]:
        ...
        if strategy == "standard":
            rollout = [
                {PAYLOAD_KEY_NAME: "deploy_all", PAYLOAD_KEY_DESCRIPTION: "Deploy to all production workers", PAYLOAD_KEY_REQUIRED: True,
                 PAYLOAD_KEY_SUCCESS_CONDITION: "health.readiness == ready"},
                {PAYLOAD_KEY_NAME: "post_deploy_verify", PAYLOAD_KEY_DESCRIPTION: "Run SLO and doctor checks", PAYLOAD_KEY_REQUIRED: True,
                 PAYLOAD_KEY_SUCCESS_CONDITION: "slo.status == healthy"},
            ]
        elif strategy == "canary":
            rollout = [
                {PAYLOAD_KEY_NAME: "deploy_canary_10pct", PAYLOAD_KEY_DESCRIPTION: "Deploy to 10% traffic slice", PAYLOAD_KEY_REQUIRED: True,
                 PAYLOAD_KEY_SUCCESS_CONDITION: "canary health ready and SLO healthy"},
                {PAYLOAD_KEY_NAME: "promote_50pct", PAYLOAD_KEY_DESCRIPTION: "Promote to 50% after checkpoint", PAYLOAD_KEY_REQUIRED: True,
                 PAYLOAD_KEY_SUCCESS_CONDITION: "no rollback triggers fired"},
                {PAYLOAD_KEY_NAME: "promote_100pct", PAYLOAD_KEY_DESCRIPTION: "Promote to full production", PAYLOAD_KEY_REQUIRED: True,
                 PAYLOAD_KEY_SUCCESS_CONDITION: "release gate remains allow/warn"},
            ]
        else:
            rollout = [
                {PAYLOAD_KEY_NAME: "shadow_deploy", PAYLOAD_KEY_DESCRIPTION: "Deploy without live execution", PAYLOAD_KEY_REQUIRED: True,
                 PAYLOAD_KEY_SUCCESS_CONDITION: "shadow decisions persisted"},
                {PAYLOAD_KEY_NAME: "shadow_compare", PAYLOAD_KEY_DESCRIPTION: "Compare shadow outputs against baseline", PAYLOAD_KEY_REQUIRED: True,
    ...
    def _build_checkpoints(self, strategy: str) -> list[dict]:
        checkpoints = [
            {PAYLOAD_KEY_NAME: "readiness", PAYLOAD_KEY_COMMAND: "engine readiness", PAYLOAD_KEY_REQUIRED: True},
            {PAYLOAD_KEY_NAME: EVIDENCE_SECTION_GATE, PAYLOAD_KEY_COMMAND: "engine gate", PAYLOAD_KEY_REQUIRED: True},
            {PAYLOAD_KEY_NAME: PAYLOAD_KEY_SLO, PAYLOAD_KEY_COMMAND: "engine slo", PAYLOAD_KEY_REQUIRED: True},
            {PAYLOAD_KEY_NAME: "doctor", PAYLOAD_KEY_COMMAND: "engine runbook doctor", PAYLOAD_KEY_REQUIRED: True},
        ]
        if strategy == "canary":
            checkpoints.extend([
                {PAYLOAD_KEY_NAME: "canary_10pct_slo", PAYLOAD_KEY_COMMAND: "engine slo", PAYLOAD_KEY_REQUIRED: True},
                {PAYLOAD_KEY_NAME: "canary_50pct_slo", PAYLOAD_KEY_COMMAND: "engine slo", PAYLOAD_KEY_REQUIRED: True},
            ])
        if strategy == "shadow":
            checkpoints.append({PAYLOAD_KEY_NAME: "shadow_evidence", PAYLOAD_KEY_COMMAND: "engine evidence build", PAYLOAD_KEY_REQUIRED: True})
        return checkpoints

    def _build_rollback_triggers(self, strategy: str) -> list[dict]:
        triggers = [
            {PAYLOAD_KEY_NAME: "release_gate_block", PAYLOAD_KEY_CONDITION: "gate.decision == block", PAYLOAD_KEY_SEVERITY: FINDING_SEVERITY_CRITICAL},
            {PAYLOAD_KEY_NAME: "slo_breach", PAYLOAD_KEY_CONDITION: "slo.status == breaching", PAYLOAD_KEY_SEVERITY: FINDING_SEVERITY_HIGH},
            {PAYLOAD_KEY_NAME: "readiness_down", PAYLOAD_KEY_CONDITION: "health.readiness != ready", PAYLOAD_KEY_SEVERITY: FINDING_SEVERITY_CRITICAL},
            {PAYLOAD_KEY_NAME: "circuit_open", PAYLOAD_KEY_CONDITION: f"{CYCLES_CIRCUIT_OPEN} > 0", PAYLOAD_KEY_SEVERITY: FINDING_SEVERITY_HIGH},
        ]
        if strategy == "canary":
            triggers.append({PAYLOAD_KEY_NAME: "canary_error_budget_exhausted",
                             PAYLOAD_KEY_CONDITION: "error_budget.exhausted_count > 0", PAYLOAD_KEY_SEVERITY: FINDING_SEVERITY_CRITICAL})
        return triggers
    ...
    def _build_commands(self, version: str, strategy: str, *, validation_mode: str) -> list[str]:
        ...
        return [
            ...
            f"python -m apps.engine.cli evidence build {mode_arg} --output-dir reports/evidence --label deploy_{version}_{strategy}",
