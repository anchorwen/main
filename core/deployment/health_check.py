...
class HealthCheckService:
    ...
    def _check_ledger_store(self) -> dict:
        ...
        return {
            PAYLOAD_KEY_NAME: HEALTH_CHECK_NAME_LEDGER_STORE,
            PAYLOAD_KEY_STATUS: HEALTH_CHECK_STATUS_OK if ok else HEALTH_CHECK_STATUS_FAILED,
        }

    def _check_risk_service(self) -> dict:
        ok = self._container.risk_service is not None
        return {
            PAYLOAD_KEY_NAME: HEALTH_CHECK_NAME_RISK_SERVICE,
            PAYLOAD_KEY_STATUS: HEALTH_CHECK_STATUS_OK if ok else HEALTH_CHECK_STATUS_FAILED,
        }
    ...
    def _check_dispatcher(self) -> dict:
        ...
        return {
            PAYLOAD_KEY_NAME: HEALTH_CHECK_NAME_DISPATCHER,
            PAYLOAD_KEY_STATUS: HEALTH_CHECK_STATUS_OK if ok else HEALTH_CHECK_STATUS_FAILED,
        }
    ...
    def _check_metrics(self) -> dict:
        ...
        return {
            PAYLOAD_KEY_NAME: HEALTH_CHECK_NAME_METRICS,
            PAYLOAD_KEY_STATUS: HEALTH_CHECK_STATUS_OK if ok else HEALTH_CHECK_STATUS_FAILED,
        }
    ...
    def _check_feedback(self) -> dict:
        ...
        return {
            PAYLOAD_KEY_NAME: HEALTH_CHECK_NAME_FEEDBACK_LOOP,
            PAYLOAD_KEY_STATUS: HEALTH_CHECK_STATUS_OK if ok else HEALTH_CHECK_STATUS_FAILED,
        }
