...
class RollbackDrillService:
    ...
    def run(
        ...
        *,
        ...
        prerequisites = self._build_prerequisites(evidence_manifest, validation_mode=validation_mode)
        ...
        passed = all(p[PAYLOAD_KEY_PASSED] for p in prerequisites) and all(c[PAYLOAD_KEY_PASSED] for c in checkpoints)
        result = {
            ...
            PAYLOAD_KEY_STATUS: RELEASE_PIPELINE_STATUS_PASSED if passed else RELEASE_PIPELINE_STATUS_FAILED,
            ...
            PAYLOAD_KEY_SUMMARY: {
                ...
                PAYLOAD_KEY_FAILED_PREREQUISITES: [p[PAYLOAD_KEY_NAME] for p in prerequisites if not p[PAYLOAD_KEY_PASSED]],
                PAYLOAD_KEY_FAILED_CHECKPOINTS: [c[PAYLOAD_KEY_NAME] for c in checkpoints if not c[PAYLOAD_KEY_PASSED]],
            },
            PAYLOAD_KEY_PREREQUISITES: prerequisites,
            PAYLOAD_KEY_STEPS: steps,
            PAYLOAD_KEY_CHECKPOINTS: checkpoints,
            PAYLOAD_KEY_RISKS: risks,
            PAYLOAD_KEY_RECOMMENDATION: ROLLBACK_RECOMMENDATION_READY if passed else ROLLBACK_RECOMMENDATION_FIX_PREREQUISITES,
        }
        if output:
        ...
        return [
            {
                ...
                PAYLOAD_KEY_PASSED: gate.get(PAYLOAD_KEY_DECISION) in {
                    RELEASE_PIPELINE_GATE_DECISION_ALLOW,
                    RELEASE_PIPELINE_GATE_DECISION_WARN,
                    RELEASE_PIPELINE_GATE_DECISION_BLOCK,
                },
            ...
            },
            {
                ...
                PAYLOAD_KEY_PASSED: readiness.get(PAYLOAD_KEY_SCHEMA_VERSION) == SCHEMA_RELEASE_READINESS,
    ...
    def _verify_evidence(self, evidence_manifest: str | None) -> dict:
        ...
        try:
            ...
            return {
                ...
                PAYLOAD_KEY_DETAIL: {PAYLOAD_KEY_FAILED_COUNT: verification.get(PAYLOAD_KEY_FAILED_COUNT, 0)},
            }
    ...
    def _build_steps(self, version: str, reason: str) -> list[dict]:
        return [
            {PAYLOAD_KEY_ORDER: 1, PAYLOAD_KEY_NAME: ROLLBACK_STEP_ANNOUNCE, PAYLOAD_KEY_DRY_RUN: True,
             PAYLOAD_KEY_DESCRIPTION: f"Announce rollback for {version}: {reason}"},
            {PAYLOAD_KEY_ORDER: 2, PAYLOAD_KEY_NAME: ROLLBACK_STEP_FREEZE_DEPLOYMENTS, PAYLOAD_KEY_DRY_RUN: True,
             PAYLOAD_KEY_DESCRIPTION: "Block new deployment executions while rollback is active"},
            {PAYLOAD_KEY_ORDER: 3, PAYLOAD_KEY_NAME: ROLLBACK_STEP_RESTORE_ARTIFACT, PAYLOAD_KEY_DRY_RUN: True,
             PAYLOAD_KEY_DESCRIPTION: "Restore previous known-good artifact or config snapshot"},
            {PAYLOAD_KEY_ORDER: 4, PAYLOAD_KEY_NAME: ROLLBACK_STEP_RESTART_SERVICES, PAYLOAD_KEY_DRY_RUN: True,
             PAYLOAD_KEY_DESCRIPTION: "Restart affected services after rollback restore"},
            {PAYLOAD_KEY_ORDER: 5, PAYLOAD_KEY_NAME: ROLLBACK_STEP_VERIFY_HEALTH, PAYLOAD_KEY_DRY_RUN: True,
             PAYLOAD_KEY_DESCRIPTION: "Run readiness, liveness, SLO, and doctor checks"},
            {PAYLOAD_KEY_ORDER: 6, PAYLOAD_KEY_NAME: ROLLBACK_STEP_CAPTURE_EVIDENCE, PAYLOAD_KEY_DRY_RUN: True,
    ...
    def _build_checkpoints(self, *, validation_mode: str | None = None) -> list[dict]:
        ...
        return [
            {PAYLOAD_KEY_NAME: ROLLBACK_CHECKPOINT_READINESS, PAYLOAD_KEY_PASSED: readiness.get(PAYLOAD_KEY_STATUS) == HEALTH_STATUS_READY,
             PAYLOAD_KEY_DETAIL: {PAYLOAD_KEY_STATUS: readiness.get(PAYLOAD_KEY_STATUS)}},
            {PAYLOAD_KEY_NAME: ROLLBACK_CHECKPOINT_LIVENESS, PAYLOAD_KEY_PASSED: liveness.get(PAYLOAD_KEY_STATUS) == HEALTH_STATUS_ALIVE,
             PAYLOAD_KEY_DETAIL: {PAYLOAD_KEY_STATUS: liveness.get(PAYLOAD_KEY_STATUS)}},
            {PAYLOAD_KEY_NAME: ROLLBACK_CHECKPOINT_SLO, PAYLOAD_KEY_PASSED: slo.get(PAYLOAD_KEY_STATUS) == SLO_STATUS_HEALTHY,
             PAYLOAD_KEY_DETAIL: {PAYLOAD_KEY_STATUS: slo.get(PAYLOAD_KEY_STATUS), PAYLOAD_KEY_FAILED_OBJECTIVES: slo.get(PAYLOAD_KEY_FAILED_OBJECTIVES, [])}},
            {PAYLOAD_KEY_NAME: ROLLBACK_CHECKPOINT_DOCTOR, PAYLOAD_KEY_PASSED: doctor.get(PAYLOAD_KEY_PASSED, False),
    ...
    def _build_risks(self, prerequisites: list[dict], checkpoints: list[dict]) -> list[dict]:
        ...
        if any(not p[PAYLOAD_KEY_PASSED] for p in prerequisites):
            risks.append({PAYLOAD_KEY_LEVEL: ROLLBACK_RISK_LEVEL_HIGH, PAYLOAD_KEY_NAME: ROLLBACK_RISK_MISSING_PREREQUISITES,
        ...
        if failed_checkpoints:
            risks.append({PAYLOAD_KEY_LEVEL: ROLLBACK_RISK_LEVEL_HIGH, PAYLOAD_KEY_NAME: ROLLBACK_RISK_CHECKPOINT_FAILURES,
                          PAYLOAD_KEY_MITIGATION: "Investigate failed rollback verification checkpoints"})
        if not risks:
            risks.append({PAYLOAD_KEY_LEVEL: ROLLBACK_RISK_LEVEL_LOW, PAYLOAD_KEY_NAME: ROLLBACK_RISK_DRY_RUN_ONLY,
