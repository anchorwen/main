...
class AlphaPromotionGate:
    ...
    def __init__(self, performance_store: AlphaPerformanceStore, policy: AlphaPromotionPolicy | None = None):
    ...
    def evaluate(self, record: AlphaRecord) -> AlphaPromotionDecision:
        ...
        if self._should_retire(metrics):
            return self._decision(record, "retire", AlphaLifecycleState.RETIRED.value, True, ["retire_threshold_breached"], metrics)
        if self._should_throttle(record.state_value, metrics):
            return self._decision(record, "throttle", AlphaLifecycleState.THROTTLED.value, True, ["throttle_threshold_breached"], metrics)
        ...
        return self._decision(record, "hold", None, False, [f"no_promotion_rule_for_state:{state}"], metrics)

    def apply(self, record: AlphaRecord, lifecycle: AlphaLifecycleService) -> AlphaPromotionDecision:
        decision = self.evaluate(record)
        if decision.approved and decision.target_state:
            lifecycle.transition(record.alpha_id, decision.target_state, reason="alpha_promotion_gate:" + decision.action)
        return decision

    def _candidate_decision(self, record: AlphaRecord, metrics: dict[str, Any]) -> AlphaPromotionDecision:
        reasons = self._base_requirements(metrics)
        if reasons:
            return self._decision(record, "hold", None, False, reasons, metrics)
        return self._decision(record, "promote", AlphaLifecycleState.BACKTEST_PASSED.value, True, ["candidate_requirements_passed"], metrics)

    def _backtest_passed_decision(self, record: AlphaRecord, metrics: dict[str, Any]) -> AlphaPromotionDecision:
        reasons = self._base_requirements(metrics)
        if reasons:
            return self._decision(record, "hold", None, False, reasons, metrics)
        return self._decision(record, "promote", AlphaLifecycleState.PAPER_TRADING.value, True, ["paper_trading_requirements_passed"], metrics)

    def _paper_trading_decision(self, record: AlphaRecord, metrics: dict[str, Any]) -> AlphaPromotionDecision:
        reasons = self._base_requirements(metrics)
        ...
        if reasons:
            return self._decision(record, "hold", None, False, reasons, metrics)
        return self._decision(record, "promote", AlphaLifecycleState.PROBATION_LIVE.value, True, ["probation_live_requirements_passed"], metrics)

    def _probation_decision(self, record: AlphaRecord, metrics: dict[str, Any]) -> AlphaPromotionDecision:
        reasons = self._base_requirements(metrics)
        if self._metric(metrics, "paper_cycles", 0) < self._policy.min_paper_cycles:
            reasons.append("paper_cycles_below_minimum")
        if reasons:
            return self._decision(record, "hold", None, False, reasons, metrics)
        return self._decision(record, "promote", AlphaLifecycleState.ACTIVE.value, True, ["activation_requirements_passed"], metrics)
    ...
    def _should_throttle(self, state: str, metrics: dict[str, Any]) -> bool:
        if state not in {AlphaLifecycleState.PAPER_TRADING.value, AlphaLifecycleState.PROBATION_LIVE.value, AlphaLifecycleState.ACTIVE.value}:
            return False
    ...
    def _decision(self, record: AlphaRecord, action: str, target_state: str | None, approved: bool, reasons: list[str], metrics: dict[str, Any]) -> AlphaPromotionDecision:
