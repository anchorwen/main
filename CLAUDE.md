...
### Iron Law #13: 全自动收口协议 (Auto-Closing Protocol) — 任务完结触发器

**触发条件**: Agent 在完成任意任务/项目后，在向人类汇报"完成"之前自动触发。包括但不限于：

- 所有 Todo 列表项已被勾选为 `completed`
- 修复/功能的核心代码修改已经完成并通过 `verify.py`
- 人类明确表示认可当前阶段的产出（"可以了"、"没问题"、"提交吧"等）
- 一轮多项目并行工作全部结束

**"任务完成"的边界定义**:
1. **自判断边界**: Agent 检查当前 TodoWrite 列表——如果所有 active items 状态均为 `completed`，视为任务完成。
2. **人类确认边界**: 人类说出"提交"、"push"、"收口"、"可以了"、"没问题了"、"完成了吗"等收口暗示——Agent 必须立即触发本协议，不得等待进一步指示。
3. **阶段完成边界**: 一轮明确的项目执行（如"项目 1+2+3"）全部交付后。

**强制收口序列** (按顺序执行，不可跳过):

1. **蓝图确认**: 检查本次修改涉及的模块蓝图是否已更新 Fix History。若未更新 → 先更新蓝图。
2. **四维闸门**: 输出 `Stability / Repairability / Decoupling / Iterability` 四维评估（#1.1），每维度标注 ↑/→/↓。
3. **Git Commit**: `git add` + `git commit`（commit message 尾部含四维评估）。若预提交钩子因实盘文件锁失败 → 使用 `--no-verify` 并注明原因（#0-bis）。
4. **Git Push**: `git push`。若 push 被拒绝（non-fast-forward）→ 中断序列，报告冲突。
5. **收口清单**: 输出 #7.1 收口块（未提交变更 / 蓝图 / Git / 本轮 commit）。

**🚨 异常中断协议 (Break-Glass Protocol)**:

收口序列在执行过程中遇到以下情况时，Agent **必须立即中断**并向人类报告，不得盲目前进：

| 异常类型 | 处理 |
|---------|------|
| Merge Conflict | `git commit`/`push` 因冲突失败 → 中断，报告冲突文件 |
| Lint/Test 失败 | `verify.py --quick` 非 PASS → 中断，报告失败项。**不得使用 `--no-verify` 绕过测试失败** |
| 蓝图合规失败 | `validate_blueprints.py` 报 FAIL → 中断，先修复蓝图 |
| 未预期未提交文件 | 有 `data/` 外的源代码文件未 staged → 中断，请人类裁决 |
| 人类中断指令 | 人类说"先别提交" / "等等" → 立即中断，执行人类指令 |

**设计原理**: #13 不新增行为规范——它把现有的 #7（蓝图更新）、#1.1（四维闸门）、#4（pre-commit）、#7.1（收口清单）编织为一个**自动触发的不可分割序列**，消除"人类需要提醒 Agent 收口"的依赖。Iron Law #-1 确保 Agent 在每次编码前自加载约束，#13 确保 Agent 在每次完成后自执行收口——首尾呼应，形成完整的 Agent 自我治理闭环。

**与 #-1 的对称性**: #-1 是"入口协议"（开始前自加载约束），#13 是"出口协议"（完成后自执行收口）。两者均不依赖人类记住复杂标签——Agent 自行判断触发条件并执行。
