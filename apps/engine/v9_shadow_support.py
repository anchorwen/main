...
class V9ParliamentAdapter:
    def build_candidate(self, feature_snapshot, proposals, control_snapshot) -> DecisionCandidate:
        if not proposals:
            ...
            up_probability = sum(p.prediction.get("up_probability", 0.5) for p in proposals) / len(
                proposals
            )
            down_probability = sum(
                p.prediction.get("down_probability", 0.5) for p in proposals
            ) / len(proposals)
            supporting_brains = [
                p.brain_id for p in proposals if p.prediction.get("direction_bias") != "neutral"
            ]
