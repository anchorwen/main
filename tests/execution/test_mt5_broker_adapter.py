...
class TestMT5BrokerAdapter:
    ...
    def test_fetch_prices_returns_mid_bid_ask(self) -> None:
        ...
        # TECH_DEBT-009: _worker 静态类型为真实 MT5Worker, 运行时是 MagicMock 探针 → cast(Any) 类型层绕过
        cast(Any, adapter._worker).symbol_info_tick.return_value = tick
    ...
    def test_fetch_prices_raises_when_tick_none(self) -> None:
        adapter = self._make_adapter()
        # TECH_DEBT-009: _worker 静态类型为真实 MT5Worker, 运行时是 MagicMock 探针 → cast(Any) 类型层绕过
        cast(Any, adapter._worker).symbol_info_tick.return_value = None
    ...
    def test_get_account_equity(self) -> None:
        adapter = self._make_adapter()
        # TECH_DEBT-009: _worker 静态类型为真实 MT5Worker, 运行时是 MagicMock 探针 → cast(Any) 类型层绕过
        cast(Any, adapter._worker).account_info.return_value = SimpleNamespace(equity=9500.0)
    ...
    def test_get_account_equity_returns_none_on_error(self) -> None:
        adapter = self._make_adapter()
        # TECH_DEBT-009: _worker 静态类型为真实 MT5Worker, 运行时是 MagicMock 探针 → cast(Any) 类型层绕过
        cast(Any, adapter._worker).account_info.side_effect = RuntimeError("mt5 crash")
    ...
    def test_count_positions(self) -> None:
        adapter = self._make_adapter()
        # TECH_DEBT-009: _worker 静态类型为真实 MT5Worker, 运行时是 MagicMock 探针 → cast(Any) 类型层绕过
        cast(Any, adapter._worker).positions_get.return_value = [MagicMock(), MagicMock()]
    ...
    def test_count_positions_zero_when_none(self) -> None:
        adapter = self._make_adapter()
        # TECH_DEBT-009: _worker 静态类型为真实 MT5Worker, 运行时是 MagicMock 探针 → cast(Any) 类型层绕过
        cast(Any, adapter._worker).positions_get.return_value = None
    ...
    def test_get_position_tickets(self) -> None:
        ...
        # TECH_DEBT-009: _worker 静态类型为真实 MT5Worker, 运行时是 MagicMock 探针 → cast(Any) 类型层绕过
        cast(Any, adapter._worker).positions_get.return_value = [p1, p2]
    ...
    def test_get_account_drawdown_pct(self) -> None:
        adapter = self._make_adapter()
        # TECH_DEBT-009: _worker 静态类型为真实 MT5Worker, 运行时是 MagicMock 探针 → cast(Any) 类型层绕过
        cast(Any, adapter._worker).account_info.return_value = SimpleNamespace(
            equity=9000.0, balance=10000.0
        )
    ...
    def test_close_position_success(self) -> None:
        adapter = self._make_adapter()
        # TECH_DEBT-009: _worker 静态类型为真实 MT5Worker, 运行时是 MagicMock 探针 → cast(Any) 类型层绕过
        cast(Any, adapter._worker).order_send.return_value = SimpleNamespace(
            retcode=10009, volume=0.1
        )
    ...
    def test_close_position_failure(self) -> None:
        adapter = self._make_adapter()
        # TECH_DEBT-009: _worker 静态类型为真实 MT5Worker, 运行时是 MagicMock 探针 → cast(Any) 类型层绕过
        cast(Any, adapter._worker).order_send.return_value = SimpleNamespace(retcode=99999)
    ...
    def test_close_position_none_response(self) -> None:
        adapter = self._make_adapter()
        # TECH_DEBT-009: _worker 静态类型为真实 MT5Worker, 运行时是 MagicMock 探针 → cast(Any) 类型层绕过
        cast(Any, adapter._worker).order_send.return_value = None
