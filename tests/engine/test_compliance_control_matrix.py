...
class TestComplianceControlMatrixService:
    ...
    def test_governance_deep_presence_warns_without_deep_release(self, tmp_path):
        c = _container(tmp_path / "data")
        _register_alpha_release(c, tmp_path, version="1.3.0", warning_count=0, validation_mode="fast")
        report = c.compliance_control_matrix.generate()
        gov3 = next(cn for cn in report["controls"] if cn["control_id"] == "GOV-003")
    ...
    def test_governance_deep_coverage_warns_for_mixed_registry(self, tmp_path):
        c = _container(tmp_path / "data")
        _register_alpha_release(c, tmp_path, version="1.4.0", warning_count=0, validation_mode="fast")
        _register_alpha_release(c, tmp_path, version="1.4.1", warning_count=0, validation_mode="deep")
        _register_alpha_release(c, tmp_path, version="1.4.2", warning_count=0, validation_mode="fast")
        report = c.compliance_control_matrix.generate()
        gov4 = next(cn for cn in report["controls"] if cn["control_id"] == "GOV-004")
    ...
    def test_control_shape(self, tmp_path):
        ...
        assert set(control) == {
            "control_id", "objective", "evidence_source", "status",
            "passed", "evidence", "gap", "remediation",
        }
...
class TestComplianceControlMatrixCLI:
    ...
    def test_cli_matrix_test_env_force_metrics(self, tmp_path, capsys):
        rc = main([
            "--base-dir", str(tmp_path), "--env", "test", "--force-metrics", "compliance-matrix",
        ])
