...
class TestReleaseGateService:
    ...
    def test_gate_accepts_fast_validation_mode(self, tmp_path):
        c = _container(tmp_path)
        report = c.release_gate.evaluate(validation_mode="fast")  # type: ignore[reportOptionalMemberAccess]
        assert report["schema_version"] == SCHEMA_RELEASE_GATE
    ...
    def test_gate_warns_non_strict_slo_breach(self, tmp_path):
        ...
        c.metrics.inc(CYCLES_ERRORS, 10)  # type: ignore[reportOptionalMemberAccess]
        report = c.release_gate.evaluate(strict=False)  # type: ignore[reportOptionalMemberAccess]
    ...
    def test_gate_blocks_strict_slo_breach(self, tmp_path):
        ...
        c.metrics.inc(CYCLES_TOTAL, 100)  # type: ignore[reportOptionalMemberAccess]
        c.metrics.inc(CYCLES_ERRORS, 10)  # type: ignore[reportOptionalMemberAccess]
        report = c.release_gate.evaluate(strict=True)  # type: ignore[reportOptionalMemberAccess]
        assert report["decision"] == "block"
        assert "slo" in report["summary"]["warning_signals"]
    ...
    def test_gate_blocks_alpha_budget_warnings_in_strict_mode(self, tmp_path):
        ...
        report = c.release_gate.evaluate(strict=True, alpha_budget_usage_report=alpha_report)  # type: ignore[reportOptionalMemberAccess]
        assert report["decision"] == "block"
    ...
    def test_gate_warns_alpha_budget_warnings_in_non_strict_mode(self, tmp_path):
        ...
        report = c.release_gate.evaluate(strict=False, alpha_budget_usage_report=alpha_report)  # type: ignore[reportOptionalMemberAccess]
        assert report["decision"] == "warn"
        assert report["allowed"] is False
        signal = [
            item
    ...
    def test_save_report(self, tmp_path):
        ...
        saved = c.release_gate.save_report(str(out))  # type: ignore[reportOptionalMemberAccess]
    ...
    def test_safe_exception_blocks(self, tmp_path):
        ...
        c.release_readiness = BrokenReadiness()  # type: ignore[reportAttributeAccessIssue]
...
class TestReleaseGateCLI:
    ...
    def test_cli_gate_blocks_on_slo_strict(self, tmp_path):
        c = _container(tmp_path)
        c.metrics.inc(CYCLES_TOTAL, 100)  # type: ignore[reportOptionalMemberAccess]
        c.metrics.inc(CYCLES_ERRORS, 50)  # type: ignore[reportOptionalMemberAccess]
        report = c.release_gate.evaluate(strict=True)  # type: ignore[reportOptionalMemberAccess]
        assert report["decision"] == "block"
