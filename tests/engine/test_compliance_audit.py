...
def _register_clean_release(container, tmp_path, version="1.0.0"):
    return _register_alpha_release(container, tmp_path, version=version, warning_count=0, validation_mode="deep")
...
def _register_alpha_release(container, tmp_path, version="1.0.0", warning_count=0, validation_mode="fast"):
    warnings = []
    if warning_count:
...
class TestComplianceAuditService:
    def test_empty_audit_warns(self, tmp_path):
        ...
        report = c.compliance_audit.generate()
    ...
    def test_audit_accepts_fast_validation_mode(self, tmp_path):
        c = _container(tmp_path)
        report = c.compliance_audit.generate(validation_mode="fast")
        assert report["schema_version"] == SCHEMA_COMPLIANCE_AUDIT
    ...
    def test_audit_summary_includes_validation_mode_counts(self, tmp_path):
        ...
        _register_alpha_release(c, tmp_path, version="2.0.0", warning_count=0, validation_mode="fast")
        report = c.compliance_audit.generate(validation_mode="fast")
    ...
    def test_audit_warns_when_registry_has_no_deep_validation(self, tmp_path):
        c = _container(tmp_path / "data")
        _register_alpha_release(c, tmp_path, version="2.1.0", warning_count=0, validation_mode="fast")
        report = c.compliance_audit.generate(validation_mode="fast")
        deep_check = next(ch for ch in report["checks"] if ch["name"] == "registry_deep_validation_present")
    ...
    def test_audit_warns_when_deep_validation_coverage_is_partial(self, tmp_path):
        c = _container(tmp_path / "data")
        _register_alpha_release(c, tmp_path, version="2.2.0", warning_count=0, validation_mode="fast")
        _register_alpha_release(c, tmp_path, version="2.2.1", warning_count=0, validation_mode="deep")
        _register_alpha_release(c, tmp_path, version="2.2.2", warning_count=0, validation_mode="fast")
        report = c.compliance_audit.generate(validation_mode="fast")
        coverage_check = next(ch for ch in report["checks"] if ch["name"] == "registry_deep_validation_coverage_complete")
        assert coverage_check["level"] == "warn"
        names = {item["name"] for item in report["summary"][PAYLOAD_KEY_GOVERNANCE_FOCUS]}
        assert names == {"registry_deep_validation_coverage_complete", "registry_deep_validation_present"}
    ...
    def test_uncertified_record_fails(self, tmp_path):
        ...
        assert any(ch["name"] == "all_registered_releases_certified" and ch["level"] == "fail"
    ...
    def test_failed_timeline_event_fails(self, tmp_path):
        ...
        report = c.compliance_audit.generate()
        assert report["status"] == "fail"
        assert any(ch["name"] == "no_failed_timeline_events" and ch["level"] == "fail"
                   for ch in report["checks"])
    ...
    def test_gate_block_fails(self, tmp_path):
        ...
        report = c.compliance_audit.generate()
        assert report["status"] == "fail"
        assert any(ch["name"] == "release_gate_not_blocking" and ch["level"] == "fail"
                   for ch in report["checks"])
    ...
    def test_alpha_budget_evidence_registered_passes(self, tmp_path):
        ...
        report = c.compliance_audit.generate()
        alpha_check = next(ch for ch in report["checks"] if ch["name"] == "alpha_budget_evidence_registered")
        warning_check = next(ch for ch in report["checks"] if ch["name"] == "alpha_budget_warnings_clear")
    ...
    def test_save_report(self, tmp_path):
        ...
        report = c.compliance_audit.generate(output=str(out))
...
class TestComplianceAuditCLI:
    ...
    def test_cli_compliance_audit_test_env_force_metrics(self, tmp_path, capsys):
        rc = main([
            "--base-dir", str(tmp_path), "--env", "test", "--force-metrics", "compliance-audit",
        ])
        out = json.loads(capsys.readouterr().out)
