...
class TestSignalOrderRequestBuilder:
    def test_builds_market_order_from_signal(self):
        builder = SignalOrderRequestBuilder(OrderSizingPolicy(base_quantity=10), default_venue="PAPER")
    ...
    def test_skips_hold_flat_and_below_threshold(self):
        builder = SignalOrderRequestBuilder(OrderSizingPolicy(base_quantity=10, min_confidence=0.6, min_strength=0.2))
    ...
    def test_limit_order_uses_signal_or_market_price(self):
        ...
        explicit = builder.build(_signal(extensions={"order_type": "limit", "limit_price": 1999.0}), {})
