...
from core.deployment.domain_keys import EVIDENCE_SECTION_ALPHA_BUDGET_USAGE
...
class TestReleasePipelineService:
    ...
    def test_run_pipeline_accepts_fast_validation_mode(self, tmp_path):
        ...
        plan = json.loads(Path(result["artifacts"][ARTIFACT_DEPLOYMENT_PLAN]).read_text(encoding="utf-8"))
        execution = json.loads(Path(result["artifacts"][ARTIFACT_DEPLOYMENT_EXECUTION]).read_text(encoding="utf-8"))
        final_audit = json.loads(Path(result["artifacts"][ARTIFACT_FINAL_AUDIT]).read_text(encoding="utf-8"))
    ...
    def test_pipeline_summary_normalizes_malformed_final_audit_governance_fields(self, tmp_path):
        ...
        assert result["summary"][PAYLOAD_KEY_GOVERNANCE_FOCUS] == [{"name": "ok"}, {"status": "warn"}]
    ...
    def test_pipeline_blocks_with_alpha_budget_usage_report_warnings(self, tmp_path):
        ...
        manifest = json.loads(Path(result["artifacts"][ARTIFACT_EVIDENCE_MANIFEST]).read_text(encoding="utf-8"))
    ...
    def test_pipeline_non_strict_carries_alpha_budget_usage_report(self, tmp_path):
        ...
        assert (Path(result["artifacts"][ARTIFACT_EVIDENCE_MANIFEST]).parent / "alpha_budget_usage.json").exists()
...
class TestReleasePipelineCLI:
    def test_cli_release_pipeline(self, tmp_path):
        rc = main(["--base-dir", str(tmp_path / "data"), "release-pipeline",
                   "--output-dir", str(tmp_path / "pipeline")])
        assert rc == 0

    def test_cli_release_pipeline_output(self, tmp_path):
        out = tmp_path / "pipeline_summary.json"
        rc = main(["--base-dir", str(tmp_path / "data"), "release-pipeline",
                   "--version", "2.0.0", "--strategy", "canary",
                   "--output-dir", str(tmp_path / "pipeline"), "--output", str(out)])
        assert rc == 0
        payload = json.loads(out.read_text(encoding="utf-8"))
    ...
    def test_cli_release_pipeline_shadow(self, tmp_path):
        rc = main(["--base-dir", str(tmp_path / "data"), "release-pipeline",
                   "--strategy", "shadow", "--output-dir", str(tmp_path / "pipeline")])
        assert rc == 0

    def test_cli_release_pipeline_fast_validation_mode(self, tmp_path):
        rc = main([
            "--base-dir", str(tmp_path / "data"), "release-pipeline",
            "--output-dir", str(tmp_path / "pipeline"),
    ...
    def test_cli_release_pipeline_test_env_force_metrics(self, tmp_path, capsys):
        ...
        rc = main([
            "--base-dir", str(tmp_path / "data"), "--env", "test", "--force-metrics",
            "release-pipeline", "--output-dir", out_dir,
        ])
    ...
    def test_cli_release_pipeline_blocks_with_alpha_budget_usage_report(self, tmp_path, capsys):
        ...
        rc = main([
            ...
            "release-pipeline",
            "--output-dir", str(tmp_path / "pipeline"),
            "--alpha-budget-usage-report", str(report_path),
        ])
    ...
    def test_cli_release_pipeline_non_strict_with_alpha_budget_usage_report(self, tmp_path, capsys):
        ...
        manifest = json.loads(Path(payload["artifacts"][ARTIFACT_EVIDENCE_MANIFEST]).read_text(encoding="utf-8"))
        assert EVIDENCE_SECTION_ALPHA_BUDGET_USAGE in manifest["summary"]["sections"]
