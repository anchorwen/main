...
class TestLedgerPersistence:
    ...
    def test_communication_record_written(self, tmp_path):
        ...
        if outcome.decision_result and outcome.decision_result.communication_record:
            today = datetime.utcnow().strftime("%Y-%m-%d")
            comm_files = list((Path(str(tmp_path / "data")) / today).glob(f"*.{stream_jsonl_suffix(LEDGER_STREAM_COMMUNICATIONS)}"))
    ...
    def test_multiple_decisions_accumulate(self, tmp_path):
        ...
        today = datetime.utcnow().strftime("%Y-%m-%d")
        decision_files = list((Path(str(tmp_path / "data")) / today).glob(f"*.{stream_jsonl_suffix(LEDGER_STREAM_DECISIONS)}"))
        total = 0
...
class TestFullDataChain:
    def test_signal_to_governance_chain(self, tmp_path):
        ...
        for mid in msg_ids[:3]:
            ...
            facade.process_event(mid, "filled", filled_quantity=0.01,
                                 price=2000.0, venue="exchange")

        # Step 3: Verify execution events on disk
        today = datetime.utcnow().strftime("%Y-%m-%d")
        exec_files = list((Path(str(tmp_path / "data")) / today).glob(f"*.{stream_jsonl_suffix(LEDGER_STREAM_EXECUTION_EVENTS)}"))
        if msg_ids:
    ...
    def test_reconciliation_after_fill(self, tmp_path):
        ...
        if outcome.decision_result and outcome.decision_result.verdict.is_allowed():
            ...
            orch.process_execution_event(
                message_id=msg_id, event_type="ack", venue="ex")
            orch.process_execution_event(
                message_id=msg_id, event_type="filled",
                filled_quantity=0.01, price=2000.0, venue="ex")

            today = datetime.utcnow().strftime("%Y-%m-%d")
            recon = c.reconciliation_service.reconcile_message(
                date_key=today, target="exec_bridge",
                message_id=msg_id, correlation_id=record_id)
