...
class TestEvidenceBundleService:
    def test_build_bundle_creates_files(self, tmp_path):
        ...
        for name in [
            "readiness", "gate", "slo", "preflight", "doctor", "diagnostics",
            "final_audit", "ops_maturity", EVIDENCE_SECTION_ENGINE_CONFIG, EVIDENCE_SECTION_MANIFEST,
        ]:
...
class TestEvidenceCLI:
    def test_cli_evidence_build(self, tmp_path):
        ...
        rc = main(["--base-dir", str(tmp_path / "data"), "evidence", "build",
                   "--output-dir", str(out), "--label", "cli1"])
        assert rc == 0
    ...
    def test_cli_evidence_build_fast_validation_mode(self, tmp_path):
        out = tmp_path / "evidence"
        rc = main([
            "--base-dir", str(tmp_path / "data"), "evidence", "build",
            "--output-dir", str(out), "--label", "cli_fast", "--validation-mode", "fast",
        ])
        assert rc == 0
    ...
    def test_cli_evidence_build_test_env_omits_runtime_metrics(self, tmp_path):
        out = tmp_path / "evidence"
        rc = main([
            "--base-dir", str(tmp_path / "data"), "--env", "test",
            "evidence", "build", "--output-dir", str(out), "--label", "cli_test_nom",
        ])
        assert rc == 0
    ...
    def test_cli_evidence_build_test_env_force_metrics_includes_runtime_metrics(self, tmp_path):
        ...
        rc = main([
            ...
            "evidence", "build", "--output-dir", str(out), "--label", "cli_test_fm",
        ])
        assert rc == 0
...
class TestEvidenceContent:
    ...
    def test_cli_evidence_build_with_alpha_budget_usage_report(self, tmp_path):
        ...
        rc = main([
            ...
            "--output-dir", str(out),
            "--label", "cli_alpha",
            "--alpha-budget-usage-report", str(report),
        ])
        assert rc == 0
