...
class TestStressMultiSymbol:
    def test_rapid_multi_symbol_ticks(self, tmp_path):
        ...
        for i in range(50):
            proc.process_tick({"symbol": symbols[i % 5], "bid": 100 + i * 0.01, "ask": 100.01 + i * 0.01})
...
class TestStressPositionExhaustion:
    def test_position_limit_blocks_after_max(self, tmp_path):
        cfg = EnvironmentConfig.development(str(tmp_path), max_open_positions=3, enable_idempotency=False)
        c = ServiceContainer(cfg).build()
        for i in range(3):
            c.position_tracker.open_position(position_id=f"p{i}", symbol="XAUUSD",
                                             side="long", quantity=1.0, entry_price=2000.0)
        orch = c.build_orchestrator()
    ...
    def test_position_freed_allows_new(self, tmp_path):
        cfg = EnvironmentConfig.development(str(tmp_path), max_open_positions=2, enable_idempotency=False)
        c = ServiceContainer(cfg).build()
        c.position_tracker.open_position(position_id="p0", symbol="XAUUSD",
                                         side="long", quantity=1.0, entry_price=2000.0)
        c.position_tracker.open_position(position_id="p1", symbol="XAUUSD",
                                         side="long", quantity=1.0, entry_price=2000.0)
        orch = c.build_orchestrator()
...
class TestStressGovernanceCascade:
    ...
    def test_mixed_health_correct_transitions(self, tmp_path):
        ...
        c.governance_rule_engine.evaluate({
            ...
            "degraded": {"health_signal": "degraded", "sample_count": 30, "composite_mean": 0.25},
            "critical": {"health_signal": "critical", "sample_count": 20, "composite_mean": 0.05},
        })
...
class TestRecovery:
    def test_save_crash_restore(self, tmp_path):
        ...
        c1.position_tracker.open_position(position_id="p1", symbol="X",
    ...
    def test_degraded_no_feedback(self, tmp_path):
        cfg = EnvironmentConfig.development(str(tmp_path), enable_feedback_loop=False, enable_idempotency=False)
        c = ServiceContainer(cfg).build()
        orch = c.build_orchestrator()
        assert orch.run_cycle({"symbol": "X"}, {"f": 1}).decision_result is not None
...
class TestContractValidation:
    ...
    def test_envelope_contract(self, tmp_path):
        ...
        if out.decision_result and out.decision_result.communication_record:
            assert ContractValidator.validate_envelope(out.decision_result.communication_record) == []

    def test_risk_context_contract(self, tmp_path):
        c, _ = _sys(tmp_path)
        c.position_tracker.open_position(position_id="p1", symbol="X",
                                         side="long", quantity=1, entry_price=100)
    ...
    def test_outcome_contract(self, tmp_path):
        c, orch = _sys(tmp_path)
        assert ContractValidator.validate_cycle_outcome(orch.run_cycle({"symbol": "X"}, {"f": 1})) == []

    def test_invalid_intent_detected(self):
        bad = type("B", (), {"intent_id": "", "candidate_id": None, "snapshot_id": "ok",
                             "symbol": "X", "venue": "MT5", "action": None, "side": None,
                             "conviction": 2.0, "event_time": "bad"})()
