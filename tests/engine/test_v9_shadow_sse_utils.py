...
from apps.engine.main_v9_shadow import SessionStreamPlan, ShadowSessionManager, render_json_output, stream_session_sse
...
def test_runtime_summary_session_sse_client_json_high_level_regression_contract(monkeypatch):
    ...
    assert manager_completed["data"]["results"][0]["operations_summary"]["message_id"] == "message_001"
    assert sse_completed["data"]["data"]["results"][0]["operations_summary"]["message_id"] == "message_001"
    assert client_completed["data"]["data"]["results"][0]["operations_summary"]["message_id"] == "message_001"
    assert rendered_json["results"][0]["operations_summary"]["message_id"] == "message_001"

    assert manager_completed["data"]["results"][0]["operations_summary"]["execution_mode"] == "targeted"
    assert sse_completed["data"]["data"]["results"][0]["operations_summary"]["execution_mode"] == "targeted"
    assert client_completed["data"]["data"]["results"][0]["operations_summary"]["execution_mode"] == "targeted"
    assert rendered_json["results"][0]["operations_summary"]["execution_mode"] == "targeted"

    assert manager_completed["data"]["results"][0]["operations_summary"]["executed_message_ids"] == ["message_001"]
    assert sse_completed["data"]["data"]["results"][0]["operations_summary"]["executed_message_ids"] == ["message_001"]
    assert client_completed["data"]["data"]["results"][0]["operations_summary"]["executed_message_ids"] == ["message_001"]
    assert rendered_json["results"][0]["operations_summary"]["executed_message_ids"] == ["message_001"]

    assert manager_completed["data"]["results"][0]["operations_summary"]["skipped_message_ids"] == ["message_002"]
    assert sse_completed["data"]["data"]["results"][0]["operations_summary"]["skipped_message_ids"] == ["message_002"]
    assert client_completed["data"]["data"]["results"][0]["operations_summary"]["skipped_message_ids"] == ["message_002"]
    assert rendered_json["results"][0]["operations_summary"]["skipped_message_ids"] == ["message_002"]

    assert manager_completed["data"]["results"][0]["operations_summary"]["skip_reasons"] == {"skip_acknowledged_message": ["message_002"]}
    assert sse_completed["data"]["data"]["results"][0]["operations_summary"]["skip_reasons"] == {"skip_acknowledged_message": ["message_002"]}
    assert client_completed["data"]["data"]["results"][0]["operations_summary"]["skip_reasons"] == {"skip_acknowledged_message": ["message_002"]}
    assert rendered_json["results"][0]["operations_summary"]["skip_reasons"] == {"skip_acknowledged_message": ["message_002"]}
...
def test_runtime_summary_iter_sse_messages_from_chunks_ignores_keep_alive_between_messages():
    messages = list(iter_sse_messages_from_chunks([
        ": keep-alive\n\n",
...
def test_runtime_summary_session_sse_client_buffer_ignores_comment_only_chunks():
    ...
    second = client.feed("event: shadowexec.progress\ndata: {\"event\": \"shadowexec.progress\", \"data\": {\"stage\": \"started\"}}\n\n")
...
def test_runtime_summary_session_sse_client_buffer_tracks_completed_terminal_message():
    ...
    first = client.feed("event: shadowexec.progress\ndata: {\"event\": \"shadowexec.progress\", \"data\": {\"stage\": \"started\"}}\n\n")
    second = client.feed("event: shadowexec.completed\ndata: {\"event\": \"shadowexec.completed\", \"data\": {\"ok\": true}}")
    finished = client.finish()
...
def test_runtime_summary_sse_server_returns_error_event_for_invalid_boolean_query_values():
    server = run_shadow_session_sse_server(ShadowSessionManager, SessionStreamPlan, host="127.0.0.1", port=0)
    try:
...
def test_runtime_summary_build_session_stream_plan_from_query_defaults_and_custom_values():
    ...
    assert default_plan == SessionStreamPlan(include_meta=True, include_stats=False, event_name_prefix="session")
    assert custom_plan == SessionStreamPlan(include_meta=False, include_stats=True, event_name_prefix="shadowexec")
...
def test_runtime_summary_build_session_stream_plan_from_query_rejects_invalid_event_prefix():
    for invalid_prefix, message in [
        ...
        try:
            build_session_stream_plan_from_query({"event_prefix": [invalid_prefix]}, SessionStreamPlan)
        except SessionStreamQueryError as exc:
            assert str(exc) == message
        else:
            raise AssertionError(f"Expected SessionStreamQueryError for event_prefix={invalid_prefix!r}")
...
def test_runtime_summary_build_session_stream_plan_from_query_rejects_invalid_boolean_values():
    for query_params in [
        ...
        try:
            ...
            raise AssertionError("Expected SessionStreamQueryError for invalid boolean value in stream plan")
