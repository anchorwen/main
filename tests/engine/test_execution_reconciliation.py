...
def _envelope(message_id, correlation_id, quantity=0):
    ...
    return CommunicationEnvelope(
        ...
        message_id=message_id, correlation_id=correlation_id, causation_id=None,
        ...
        priority=CommunicationPriority.NORMAL, payload=payload,
    )
...
def _result(message_id):
    return DispatchResult(
        ...
        dispatch_id=f"d_{message_id}", message_id=message_id,
        status=DispatchStatus.TRANSPORT_DELIVERED,
        recorded_at=datetime(2026, 4, 24, 12, 0, 1),
        target="exec_bridge", adapter_name="stub_adapter",
...
class TestReconciliationService:
    ...
    def test_matched_fill(self, tmp_path):
        svc = self._build(tmp_path,
            [{"mid": "m1", "cid": "c1", "qty": 100}],
            [{"mid": "m1", "cid": "c1", "type": "ack"},
             {"mid": "m1", "cid": "c1", "type": "filled", "fq": 100}],
        )
        r = svc.reconcile_message(date_key="2026-04-24", target="exec_bridge", message_id="m1", correlation_id="c1")
...
class TestInspectionServiceWithExecutionTimeline:
    ...
    def test_trace_without_execution_reader(self, tmp_path):
        store = JsonlLedgerStore(str(tmp_path))
        CommunicationRecordWriter(ledger_store=store).write_record(_envelope("m1", "c1"), _result("m1"))
    ...
    def test_correlation_delivery_summary_includes_execution_stats(self, tmp_path):
        ...
        trace = svc.get_correlation_trace(date_key="2026-04-24", target="exec_bridge", correlation_id="c1")
