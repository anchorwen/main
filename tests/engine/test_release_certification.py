...
class TestReleaseCertificationCLI:
    def test_cli_certify_and_verify(self, tmp_path):
        ...
        pipeline_path.write_text(json.dumps(pipeline), encoding="utf-8")
        rc = main(["--base-dir", str(tmp_path / "data"), "release-cert", "certify",
                   "--pipeline", str(pipeline_path), "--approver", "qa", "--output", str(cert_path)])
        assert rc == 0
        rc_verify = main(["--base-dir", str(tmp_path / "data"), "release-cert", "verify",
    ...
    def test_cli_verify_tampered_certificate(self, tmp_path):
        ...
        rc = main(["--base-dir", str(tmp_path / "data"), "release-cert", "verify",
                   "--certificate", str(cert_path)])
        assert rc == 1
    ...
    def test_cli_certify_includes_alpha_budget_evidence(self, tmp_path):
        ...
        pipeline_path.write_text(json.dumps(pipeline), encoding="utf-8")
        rc = main(["--base-dir", str(tmp_path / "data"), "release-cert", "certify",
                   "--pipeline", str(pipeline_path), "--output", str(cert_path)])
        assert rc == 0
    ...
    def test_cli_certify_and_verify_test_env_force_metrics(self, tmp_path):
        ...
        rc = main([
            ...
            "release-cert", "certify",
            "--pipeline", str(pipeline_path), "--approver", "qa", "--output", str(cert_path),
        ])
        assert rc == 0
        rc_v = main([
            "--base-dir", str(tmp_path / "data"), "--env", "test", "--force-metrics",
            "release-cert", "verify", "--certificate", str(cert_path),
        ])
        assert rc_v == 0
        cert = json.loads(cert_path.read_text(encoding="utf-8"))
