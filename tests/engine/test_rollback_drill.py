...
class TestRollbackDrillService:
    def test_run_drill_passes(self, tmp_path):
        ...
        result = c.rollback_drill.run(version="1.0.0", reason="test")
    ...
    def test_drill_with_verified_evidence(self, tmp_path):
        ...
        built = c.evidence_bundle.build_bundle(str(tmp_path / "evidence"), label="rb1")
        result = c.rollback_drill.run(evidence_manifest=built["manifest_path"])
        assert result["passed"] is True
    ...
    def test_drill_detects_tampered_evidence(self, tmp_path):
        c = _container(tmp_path / "data")
        built = c.evidence_bundle.build_bundle(str(tmp_path / "evidence"), label="rb2")
    ...
    def test_save_output(self, tmp_path):
        ...
        result = c.rollback_drill.run(output=str(out))
...
class TestRollbackDrillCLI:
    ...
    def test_cli_rollback_drill_test_env_force_metrics(self, tmp_path, capsys):
        rc = main([
            "--base-dir", str(tmp_path), "--env", "test", "--force-metrics", "rollback-drill",
        ])
    ...
    def test_cli_rollback_drill_with_evidence(self, tmp_path):
        c = _container(tmp_path / "data")
        built = c.evidence_bundle.build_bundle(str(tmp_path / "evidence"), label="cli")
        rc = main(["--base-dir", str(tmp_path / "data"), "rollback-drill",
                   "--evidence-manifest", built["manifest_path"]])
        assert rc == 0

    def test_cli_rollback_drill_bad_evidence(self, tmp_path):
        rc = main(["--base-dir", str(tmp_path), "rollback-drill",
                   "--evidence-manifest", str(tmp_path / "missing.json")])
        assert rc == 1
