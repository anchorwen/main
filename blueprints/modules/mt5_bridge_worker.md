...
|--------|------|-------------|------------|
| FIX-20260819-002 | 2026-08-19 | **TECH_DEBT-010 Blueprint B+C: Journal Firewall 跨域拒签 + ZMQ endpoint 必选 (Storm Forensics)**. (B) `_append_journal` 唯一写盘 chokepoint 前插 Firewall — XAU 账本仅收 XAUUSD(c), BTC 仅收 BTCUSDc; 域由 `--journal-domain` 显式 / `--default-symbol` 前缀推导, 启动打印 firewall_armed/disarmed; 跨域记录拦截写 `cross_domain_warnings.jsonl` 绝不进 SSOT (7/20-8/4 BTC 227 modify 串台根除). (C) `--zmq-order-endpoint` 默认 5556 → 必填 fail-fast; `run_zmq_worker(order_endpoint)` required — 多品种架构无默认端口兜底. ReB: `CROSS_ASSET_JOURNAL_CONTAMINATION` / `ZMQ_DEFAULT_PORT_FALLBACK`. | RC-12 |
