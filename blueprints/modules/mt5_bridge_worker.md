...
|--------|------|-------------|------------|
| FIX-20260821-002 | 2026-08-21 | **TECH_DEBT-007 (DQAF-20260821-001, IC 批准): `_derive_label` watchdog shortcode 归一化 (SSOT 2 段)**. 原 L1054-1059 内联提取 `split("_", 3)[:3]` 产 3 段 (watchdog:hesitation_18c_no), 与引擎各生产者 2 段 (watchdog:hesitation_18c) 漂移 — 同 deal 桥临时标签与覆写方 byte-不不一致. 收敛为 import `core.runtime.close_label.watchdog_shortcode` (纯 stdlib, 无新跨层依赖) → provisional bridge 标签与 SSOT 契约逐字节一致. 零 PnL/行为语义变化 (仅 label 短码格式). ReB: `CLOSE_LABEL_MULTI_PRODUCER_DIVERGENCE`. | RC-06 |
