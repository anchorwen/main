...
| FIX-20260529-051 | 2026-05-30 | cursor-agent | — | Last Mile Protocol Phase 2 — small-file FTC conversion: (1) contract_groups.py 4 LOG sites → log_and_continue(). (2) exit_watchdog.py 1 LOG → log_and_continue(). (3) strategy_line.py 1 DEGRADE → FTC(DEGRADE). (4) market_ingress.py 2×2 MT5 IPC → FTC(CRASH) with pre-init. (5) mt5_bridge_worker.py 1 MT5 IPC → FTC(CRASH) with pre-init. FTC paradigm now covers ~25 sites across 8 files. | RC-07 |
