...
| FIX-20260529-051 | 2026-05-30 | cursor-agent | — | Last Mile Protocol Phase 2: (1) contract_groups.py 4 LOG → log_and_continue(). (2) exit_watchdog.py 1 LOG → log_and_continue(). (3) strategy_line.py 1 DEGRADE → FTC(DEGRADE). (4) market_ingress.py 4 MT5 IPC → FTC(CRASH). (5) mt5_bridge_worker.py 1 MT5 IPC → FTC(CRASH). (6) live_cycle.py DEGRADE (ManagementBrainInference, DailyFeature, CapitalAllocator, EntryBrain:SwingBrain), LOG (PnLLedger:settle, PositionState:periodic_save), CRASH (PositionState:save_after_register). ~42 sites now FTC across 8 files (added MIA close journal/reentry guard/save, feature compute micro/daily, feature store/freshness). | RC-07 |
