...
| FIX-20260630-198 | 2026-06-30 | cursor-agent | — | **L2: Fix KeyError 'queued' crash + ImportError live_alert_hub** (DQAF-20260630-198). `strategy_evaluator.py`: two early-return dicts (bootstrap_degraded, session_time_blocked) missing `queued` + `active_strategies` keys → key parity restored. `live_cycle.py:3527-3529`: `.get()` defaults replace `[]` indexing. `live_intent_loop.py:2353`: `core.feedback.live_alert_hub` → `core.observability.live_alert_hub`. Crash chain: session_time_blocked → early return → KeyError → shutdown → wrong import → fatal_error. | L2 — producer-consumer key contract violation; RC-06 — interface contract not honored |
