...
| FIX-20260629-171 | 2026-06-29 | cursor-agent | — | **Strategy mode runtime enforcement**: StrategyLineConfig新增`mode`字段 (live/probation/shadow), strategy_builder从YAML读取, strategy_evaluator强制执行: mode=probation且无brain达到governance probation+状态→强制shadow模式(虚单不实盘). 闭合DQAF-20260609-011中Cut 4微量化交易仍允许candidate brain实盘交易的缺口. | L2 — YAML `mode`字段纯文档化, 零运行时门禁 |
