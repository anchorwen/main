...
| FIX-20260703-061 | 2026-07-03 | cursor-agent | — | **L2: FIX-060 incomplete — expand active brain filter in strategy_evaluator.py Cut 4** (DQAF-20260703-061). `strategy_evaluator.py:608`: Cut 4 governance degradation gate `_live_count` expanded from `status == "live"` to `status in ("live", "probation")`. FIX-060 fixed `strategy_line.py` but missed this independent code path. btc_swing_h1 (sole brain V12_H1_15 = probation) had `_live_count=0` every cycle → permanent `[degraded: no_live_brains]` + volume=0.01. Cut 4-bis automatically corrected. Historical: FIX-20260629-174 also missed strategy_evaluator.py — systemic incomplete-fix risk from cross-file duplicate gate logic. | L2 — incomplete fix: cross-file duplicate gate repaired in only one of two paths |
| FIX-20260630-197 | 2026-06-30 | cursor-agent | — | **L3: Remove √t ATR scaling in `dynamic_sl_tp.py`** (DQAF-20260630-197). Removed `current_atr = current_atr * math.sqrt(timeframe_mult)` (lines 139-140) — SL/TP distances were inflated 3.46–16.97× for non-M5 strategies. H4: 6.93×, H1: 3.46×, M30: 2.45×, M15: 1.73×, D1: 16.97×. Affected all 8 non-M5 BTC+XAU strategies. Root cause: √t scaling double-counts ATR volatility; training labels use M5 ATR, live SL must match. Fix: ATR preserved at native M5 scale; timeframe_mult retained for backward compat. Forensic evidence: 3 independent scripts, BTC match 0.6% of √t computation. See FIX_REGISTRY for full analysis. | L3 — design defect: √t on already-volatility-normalized ATR |
