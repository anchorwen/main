...
| FIX-20260803-002 | 2026-08-03 | cursor-agent | — | **L3: BTC 特征计算图绝对统一 (M1 战役一 / IC 最高批准)**. `btc_feature_augmenter.augment()` 状态剥离 → 模块级纯函数 `_assemble_41`/`assemble_41_vector`/`assemble_41_series` (prev-OU/Hurst 显式入参, 消除状态泄漏); live 与历史回放共享同一装配代码脉络. `core/training/feature_replay.py` 历史回放唯一入口 (OHLC → daily_swing/micro 组件 → 共享装配). `feature_service._IMPLEMENTED_SCHEMAS` 注册 `btc_macro_flow_46`. 9 个 bit-identical 硬断言测试. XAU 零行为回退. | state-leak (RC-03) |
