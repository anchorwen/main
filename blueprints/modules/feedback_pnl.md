...
| FIX-20260819-003 | 2026-08-19 | cursor-agent | (pending) | **TECH_DEBT-008 清偿: live_shadow_ensemble `cross_assets` 多余 str() 转换** — `_XAU_CROSS_CSVS` 值已是 Path, 包 `str()` 与注解 dict[str,Path] 类型冲突 → 去 `str()`, 注解宽化 `dict[str, str | Path]` (Path.exists() 直接判定), 行为零变化. | L1 — 类型声明与实现不一致 |
