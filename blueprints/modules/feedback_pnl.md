...
| FIX-20260628-060 | 2026-06-28 | cursor-agent | ff9239ed | DQAF-060 L3: Event Sourcing semantic conflation — load_from_stream() now filters by event_type (only SignalSettled enters _settled), PF sentinel 999.0 removed (float('inf') preserved), _data_source lineage honesty (live_journal vs pnl_store), augmented journal freshness integrated into daily_ops pipeline | state-leak |
| FIX-20260623-068 | 2026-06-23 | cursor-agent | — | **DQAF-068 P0: PnL Ledger SSOT Rebuild — V12 Blind-Flight Rescue**. BTC_Swing_V12_H1_Survival (only live BTC brain, 56 trades) had 0 PnL records in `brain_pnl_ledger.json` despite 1,525 events in `ledger_events.jsonl` (37 SignalSettled). Root cause: the JSON VIEW was stale — `load_from_stream()` correctly reconstructs V12 with 1,530 records but the on-disk JSON had never been regenerated. Fix: rebuilt PnL ledger VIEW from immutable event stream SSOT using `BrainPnLStore.load_from_stream()` + `save()`. V12 now visible to DynamicBrainWeighter. No config change needed — brain config already had `magic: 90411`. | L2 — stale JSON view: event stream (SSOT) had complete V12 data, but the JSON projection was never regenerated after `load_from_stream()` was implemented in FIX-20260611-022 |
...
| FIX-20260819-003 | 2026-08-19 | cursor-agent | 4990c1d0 | **TECH_DEBT-008 清偿: live_shadow_ensemble `cross_assets` 多余 str() 转换** — `_XAU_CROSS_CSVS` 值已是 Path, 包 `str()` 与注解 dict[str,Path] 类型冲突 → 去 `str()`, 注解宽化 `dict[str, str | Path]` (Path.exists() 直接判定), 行为零变化. | L1 — 类型声明与实现不一致 |
