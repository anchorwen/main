...
|--------|------|--------|--------|---------|------------|
| FIX-20260804-006 | 2026-08-04 | cursor-agent | — | **L2: XAU 退化脑 The Great Purge — 6 脑物理冻结 (DQAF-20260804-006, Sev 2 / IC 雷霆裁决)**. D1 特征毒井 (live_intent_loop.py:759 H1 硬编码 XAU D1 CSV → BTC 进程跨写 XAU D1 bar 至 data/raw/xauusdc_d1_merged.csv, 07-04 起尾部全 BTC 价) → 全 swing 脑 D1_* 特征投毒 → 输出退化 SHORT 锁死 (H1_Exec_A 100% SHORT 155/155, M30_V5 99.4% test-acc 0.3396≈随机). IC 裁决: 否决方向均衡约束 (烂脑加约束=逼迫随机 LONG), 物理冻结 6 脑 config → retired + vote_weight 0 + enabled:false. (D1 毒井根治见 runtime_live.md FIX-20260804-007.) | L2 — 特征毒井跨资产投毒致输出退化, 物理冻结为终局裁决 |
| FIX-20260804-005 | 2026-08-04 | cursor-agent | 32339723 | L3: BTC_Flow46_V1_SHORT Runtime evaluator wiring (T30-02 / IC mandate). New TransferResidualBrainAdapter composes y=y_A(frozen base)+r(OFI residual) bit-identical to ResidualTransferLearner; BrainFactory dispatch on transfer.kind=freeze_and_residual (Method A, brain_type keeps signal semantics); brain_config.py emits training_params+transfer; Flow46 config +objective/activation_threshold/frozen_base_artifact_path. ReB: CONTRACT_WITHOUT_RUNTIME_EVALUATOR. | contract-violation |
