...
| FIX-20260819-004 | 2026-08-19 | cursor-agent | 0895ece3 | **TECH_DEBT-017 清偿 (DQAF-20260819-004): group_consensus CorrelationTracker:penalty 降级路径作用域陷阱** — FTC(DEGRADE) 吞 get_correlation_penalty 异常后 dynamic_volume 永不绑定 → 块外 return dict UnboundLocalError. 修复: FTC 块前预绑定 `dynamic_volume = raw_volume` (与 else 分支一致, correlation penalty 不可用退回无惩罚仓位). 回归锁: test_tech_debt_017_scope_safety.py behavioral + static. ReB: FTC_SCOPE_TRAP_UNBOUNDLOCAL. | L3 — FTC 契约吞异常 × 调用点未预绑定变量 |
