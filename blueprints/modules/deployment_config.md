...
| FIX-20260826-005 | 2026-08-26 | cursor-agent | (pending backfill) | **FIX-20260826-005 (IC 2026-08-26 Mandate #3, DQAF-20260826-005): 特区线窄门配置接线**. `configs/live_btc.yaml` `btc_expected_r_m15` strategy_line 新增 3 键: `execution_zone: live_fire_vanguard` + `allowed_brain_ids: [BTC_Expected_R_V4_SHORT]` + `min_zone_rho: 0.05` — 驱动 `strategy_builder.py` narrow-gate (runtime_live.md). 仅授权 V4_SHORT (candidate/shadow ∩ ρ≥0.05) 穿透, 其余期望收益脑 (V4_LONG ρ=0.0445/V5_LONG/V5_SHORT ρ=null) 在 build 期 fail-closed. ~13 既有配置一致性 WARN (XAU swing SL/TP 错配 + V4_Brain label_contract.aligned_with=null) 非本次引入. **激活前提**: 当前运行引擎 14864 (zombie-purge 冷重启) 采用无 `execution_zone` 旧 config → 窄门在下次重启才焊接 (未获 IC 重启授权, 未执行). | L1/L2 — 高风险 candidate 脑 (V4_SHORT) 过不了正常 OOS 门禁, 需特区窄门破局 |
| FIX-20260821-008 | 2026-08-21 | cursor-agent | — | **P2 MetaExit 战役 (DQAF-20260821-003, IC 裁决二 The Shadow Deployment): per-asset MetaExit 模型路径常量**. `core/deployment/path_defaults.py` 新增 `META_EXIT_MODEL_XAU_PATH = "data/models/meta_exit_model_v3_xau.txt"` + `META_EXIT_MODEL_BTC_PATH = "data_btc/models/meta_exit_model_v3_btc.txt"` (v3 19-dim 重训产物), 保留 legacy `META_EXIT_MODEL_PATH` (v1 8-dim, 向后兼容). 运行时分派见 runtime_live.md. ReB: CROSS_ASSET_DEFAULT_SILENT_MISPAIR. | RC-06 — contract-violation: 无 per-asset 模型路径 SSOT |
