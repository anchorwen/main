...
| FIX-20260826-005 | 2026-08-26 | cursor-agent | c1de720c | **FIX-20260826-005 (IC 2026-08-26 最高开火令 Mandate #3, DQAF-20260826-005): 敢死队家族同池 — 共享 $50 生死状 (The Shared Bloodline)**. `live_fire_breaker.py` 新增 `LIVE_FIRE_TRACKED_MAGICS=(90601,90452)` 单点扩展点 (90601 XAU Micro Scaler v2 + 90452 BTC V4_SHORT 特区); `aggregate_live_fire_drawdown` 参数化 `magic:int|None` / `magics:tuple|None` — 单值向后兼容旧审计 (magic=90601), `magics` 显式覆盖, 两者皆缺省聚合全家族; `per_tree[dir].{realized_pnl_usd,n_closed,breached}` 顶层键保持向后兼容 (审计直接读), 另含 `by_magic` 分 magic 明细. **⚠️ 注**: 本 fixes 只把 V4_SHORT 的 PnL **计数**并入家族血槽 (一损俱损的记账侧); **停止侧未接线** — `is_breaker_open` 仅门禁 `_dispatch_live_fire_micro_scaler`, V4_SHORT 常规派发不经此门禁 (另立案卷). 回归锁 G10-G12 (跨树多 magic 聚合 / 单值向后兼容 / 缺省家族聚合). | L1/L2 — 敢死队是"一个计划"跨品种共享 $50, 非每 magic 各一血槽 (IC Decoupling 裁决) |
| FIX-20260826-006 | 2026-08-26 | cursor-agent | pending | **FIX-20260826-006 (IC 2026-08-26 最高阻断令, DQAF-20260826-006): 敢死队特区停止侧闭环 — check_vanguard_breaker**. `live_fire_breaker.py` 新增 `LIVE_FIRE_MAX_DRAWDOWN_USD=50.0` (生死状金额单点扩展点, 与 micro_scaler config 默认一致, 不随 yaml 漂移) + `check_vanguard_breaker(execution_zone, base_dir)`. 语义: **边界控制** — 仅 `execution_zone=="live_fire_vanguard"` 强制校验, 非特区恒 False 绝不误杀正常 live 策略; **闭环** — flag 已存在→True (fail-closed) / flag 缺但全局聚合击穿→自探测 + 幂等双写 flag 再 True (XAU 静默时 BTC 特区击穿也拦, STOP 半侧彻底闭环); base_dir 单树隔离 (非全局树仅查自身树 flag/账本, 防测试误读全局生产账本). 回归锁: test_live_fire 新增 4 单元 (非特区恒 False / 特区 flag 停 / 自感知击穿写 flag+停 / 未击穿放行). | L2 逻辑防护 — STOP 半侧缺口: 生死状只统计不拦截, 常规派发链无 `is_breaker_open` 门禁 |
| FIX-20260826-007 | 2026-08-26 | cursor-agent | pending | **FIX-20260826-007 (IC 2026-08-26 批准, DQAF-20260826-007: Sev 3, L2): 敢死队特区出舱 — runtime brain 投影透传 training_metrics**. 重启后 `vanguard_narrow_gate_failclosed: admitted=[]` — 窄门把 V4_SHORT 判 `rho_below_threshold, detail=None`. 真因: `live_intent_loop.py` 运行时 brain dict 白名单投影 (13 字段) 不含 `training_metrics` → 窄门读 `_b["training_metrics"]["spearman_rho"]`=None → `rho>=0.05` 不成立 → fail-closed (train/serve skew — serve 投影未携带下游消费方字段). 特效 (单点): `live_intent_loop.py` 投影增补 `"training_metrics": entry.get("training_metrics")` 透传, 窄门正常取 spearman_rho=0.0596 ≥ 0.05 → V4_SHORT 出舱. 语义保持: 字段缺失仍 fail-closed. vote_weight 无涉 (adapter 自存 `_brain_entry`). 回归锁: test_strategy_builder 新增 `test_narrow_gate_fail_closed_on_absent_training_metrics` (缺字段→fail-closed). | L2 逻辑修复 — serve 投影字段缺失: `_b["training_metrics"]` None 致窄门误拒 V4_SHORT, 特区假死 |
