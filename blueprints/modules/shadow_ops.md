...
| FIX-20260826-005 | 2026-08-26 | cursor-agent | (pending backfill) | **FIX-20260826-005 (IC 2026-08-26 最高开火令 Mandate #3, DQAF-20260826-005): 敢死队家族同池 — 共享 $50 生死状 (The Shared Bloodline)**. `live_fire_breaker.py` 新增 `LIVE_FIRE_TRACKED_MAGICS=(90601,90452)` 单点扩展点 (90601 XAU Micro Scaler v2 + 90452 BTC V4_SHORT 特区); `aggregate_live_fire_drawdown` 参数化 `magic:int|None` / `magics:tuple|None` — 单值向后兼容旧审计 (magic=90601), `magics` 显式覆盖, 两者皆缺省聚合全家族; `per_tree[dir].{realized_pnl_usd,n_closed,breached}` 顶层键保持向后兼容 (审计直接读), 另含 `by_magic` 分 magic 明细. **⚠️ 注**: 本 fixes 只把 V4_SHORT 的 PnL **计数**并入家族血槽 (一损俱损的记账侧); **停止侧未接线** — `is_breaker_open` 仅门禁 `_dispatch_live_fire_micro_scaler`, V4_SHORT 常规派发不经此门禁 (另立案卷). 回归锁 G10-G12 (跨树多 magic 聚合 / 单值向后兼容 / 缺省家族聚合). | L1/L2 — 敢死队是"一个计划"跨品种共享 $50, 非每 magic 各一血槽 (IC Decoupling 裁决) |
