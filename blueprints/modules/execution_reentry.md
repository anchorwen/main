...
| FIX-20260703-003 | 2026-07-03 | cursor-agent | — | **DQAF-061: BTC Deadlock Trifecta Melt**. C1: bleed_stop dual escape channels (12-bar TTL + abs-conf ≥0.65) in reentry_guard.py. C2: min_p_win 0.45→0.40 in live_btc.yaml for btc_swing + btc_swing_h1. C3: btc_macro cold-start thermal alignment probes in feature_assembler.py + live_cycle.py. | RC-07 — missing-validation |
| FIX-20260705-008 | 2026-07-05 | cursor-agent | — | **L3: Bleed_stop adverse selection — replace confidence momentum with absolute threshold**. The old requirement `new_confidence >= exit_confidence + 0.15` (BTC) systematically blocked stable high-confidence signals (structural Alpha marker in tree models) while allowing volatile confidence-spike signals (noise-overfitting) through. BTC post-restart data: bleed_stop-blocked trades 75% forward-correct at +60min vs approved trades 56%. Fix: (a) replaced improvement requirement with `_BLEED_ABSOLUTE_CONFIDENCE = 0.50` (matching TTL unlock floor), (b) marked `bleed_penalty_override`/`reentry_bleed_penalty` as DEPRECATED in reentry_guard.py, live_cycle.py, and live_btc.yaml. Cooldown (600s BTC / 180s XAU) and price confirmation checks retained. | L3 — adverse selection: confidence momentum gate filtered out good signals, selected for noise |
| FIX-20260629-195 | 2026-06-29 | cursor-agent | — | **V6 Exit Priority Infrastructure — +8 canonical exit reasons**: Added BASKET_TP, ZSCORE_FLIP, REGIME_COLLAPSE, CB_EMERGENCY, CB_SLOW_BURN, PREMISE_INVALID, BREAKEVEN_DEFENSE, DRAWDOWN_LOCK to ExitReason enum with cooldown tiers and classify() mappings. New reasons support the 7-level exit priority pipeline (P1-P7). | RC-12 |
