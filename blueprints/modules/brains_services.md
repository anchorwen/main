...
| FIX-20260804-005 | 2026-08-04 | cursor-agent | — | **L3: T30② BrainFactory 分派方案 A — transfer 元数据驱动 adapter 选择 (IC 最高行动令)**. `BrainFactory.build()` 读 `transfer.kind==freeze_and_residual` 元数据 → 实例化 `TransferResidualBrainAdapter` (base+residual 组合); 无 transfer 块的脑 (XAU 全部, V4/V5 expected_r 塔) 路由不变. brain_type 保留信号语义 (expected_r_short), 不发明新 brain_type — IC 裁决的架构语义: brain_type 描述输出信号, transfer 块描述物理结构. `adapter_cls: type` 显式注解过 mypy 双分支赋值. | RC-06 — contract-violation: CONTRACT_WITHOUT_RUNTIME_EVALUATOR (BrainFactory 无 transfer 元数据分派, Flow46 脑只能落 LightGBMBrainAdapter 维度守卫死区) |
