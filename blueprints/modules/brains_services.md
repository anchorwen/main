...
| FIX-20260528-023 | 2026-05-28 | cursor-agent | — | `train_swing_v9.py` brain config output: added `schema_version`, `magic`, `artifact_path`, `training_horizon` fields. Swing_V9 configs silently skipped by `_load_brain_entries_from_dir()` at live_intent_loop.py:166 because `schema_version` field was absent — only files with `== "brain_registry_entry.v1"` pass the gate. Also missing `magic` (MT5 dispatch), `artifact_path` (model integrity), `training_horizon` (BrainEntry schema). Fixed both Swing_V9 configs retroactively. | RC-09 |
