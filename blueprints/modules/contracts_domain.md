...
| FIX-20260708-003 | 2026-07-08 | cursor-agent | — | **DQAF-20260708-003: PositionClosed +close_price_source/pnl_status provenance**. Deal-selection SSOT fix (position_close_adapter blind `deals[0]` picked the DEAL_ENTRY_IN opening deal → fabricated break-even at entry price on every full close). Added two provenance fields to `PositionClosed` (`close_price_source`, `pnl_status`) + `to_journal_entry()` output so a non-broker-verified close self-declares. Backward-compatible via `""` defaults. | L3 — close-event contract carried no provenance, so a fabricated close price was indistinguishable from a real MT5 exit-deal price |
