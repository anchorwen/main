...
| FIX-20260715-015 | 2026-07-15 | cursor-agent | — | **L2-β: Governance state load retry loop — replace `except: pass`**. `live_cycle.py` governance_state load now retries 3× with 50ms backoff. Structured warning logged on exhaustion with path/error/mitigation detail. Defensive telemetry alert via DataHealthService (getattr-guarded). Fixes 21.3% silent governance load failures causing cold_explore_neutral (p_win=0.50). | L2 — silent data loss: `except: pass` swallowed JSON parse/IO errors |
| FIX-20260712-002 | 2026-07-12 | cursor-agent | — | **Phase 1 Layer 1: GovernanceService transition_log integrity gate**. `save()` now validates all transition_log entries against VALID_TRANSITIONS before persistence. Invalid transitions (e.g. live→shadow) raise InvalidTransitionError and block save. Part of 4-layer config defense system (FIX-20260712-002). | RC-06, RC-09 |
| FIX-20260701-204 | 2026-07-01 | cursor-agent | — | **L3: FIX-20260630-202 Contract 2 Completion — Duplicate Rule 85 in `_promote_shadow_brains()`**. Rule 85 (auto_promote_shadow_to_probation) was duplicated in two files: `governance_rule_engine.py` (BTC path, already fixed by FIX-20260701-203) and `governance_scheduler.py::_promote_shadow_brains()` (XAU daily_ops path, unfixed). The duplicate lacked macro exemption — H4_V3 (456 signals, 0L/456S) passed all criteria but was blocked by `long_count < 5` hard check. Added BrainRegistry macro probe to `_promote_shadow_brains()` mirroring the exemption in `governance_rule_engine.py`. Additionally fixed `daily_ops_scheduler.py` missing `base_dir` pass-through (defaulted to `data_btc` → XAU ShadowTracker read from wrong directory). | L3 — duplicated rule implementation unsynchronized (architecture split: cloud scheduler_service vs local daily_ops path) |
| FIX-20260701-203 | 2026-07-01 | cursor-agent | — | **L1: FIX-20260630-202 Contract 2 AttributeError — `entry.timeframe` → `entry.raw.get("timeframe")`**. BrainEntry dataclass has no `timeframe` attribute (stored in `raw` dict). The `entry.timeframe` access raised `AttributeError`, which was NOT in the except tuple `(RuntimeError, ValueError, KeyError, TypeError, OSError)`. Condition function silently crashed → `is_macro` stayed False → legacy diversity check ran → H4_V3 promotion blocked despite 445 directional signals. Added `AttributeError` to except clause as defense-in-depth. | L1 — attribute access not validated against dataclass |
| FIX-20260630-202 | 2026-06-30 | cursor-agent | — | **L3: Macro-Regime Diversity Exemption — `auto_promote_shadow_to_probation`**. H4/D1 timeframes produce directionally-monopolistic signals by design (genuine trend followers SHOULD output 100% SHORT in downtrend). Added BrainRegistry contract_group/timeframe probe to relax long≥5 AND short≥5 diversity requirement for macro timeframes. Macro brains: 50+ signals + confidence≥0.50 suffices. Non-macro: legacy diversity check preserved. DQAF-20260630-202. | toxic-diversity-gate |
| FIX-20260628-162 | 2026-06-28 | cursor-agent | — | **L2: auto_demote_degraded last-live-brain guard**. Modified `_demote_condition` to check `len(live_brains) <= 1` before demoting. Demoting the sole live brain triggers DQAF-059 (0 live → fail-closed p_win=0.40 → all trading blocked). A degraded live brain still trades with guard parameters; zero live brains blocks the entire pipeline. Fixes FIX-20260628-161 oscillation: V4 restored to live by SSOT reconcile, then immediately demoted back to probation by auto_demote_degraded. | L2 — governance rule lacked minimum-live-brain floor check |
...
| FIX-20260629-171 | 2026-06-29 | cursor-agent | — | **Ghost registration defense-in-depth**: Cleaned 83/96 ghost entries from BTC governance transition_log. GovernanceService 新增 `_valid_brain_ids` 白名单 + `resolve_valid_brain_ids()` 静态方法 + `register_brain()` 防御纵深拒绝 + `load()` 自动解析. 闭合 FIX-20260628-168 仅覆盖 governance_scheduler.py 的缺口. | L2 — 5+ 注册路径 (daily_ops, scheduler_service, brain_lifecycle_manager) 均未设防 |
