# Risk / Portfolio

## Purpose
Cross-strategy portfolio-level risk control: VaR/CVaR computation, correlation penalty, net-out detection, and capital allocation optimization across concurrent positions.

## Key Files
| File | Role |
|------|------|
| `core/execution/portfolio_risk.py` | `PortfolioRiskController` — cross-strategy limits, VaR, correlation |
| `core/execution/capital_allocator.py` | `AllocationDecision`, `compute_optimal_group_weights()` |
| `core/metrics/portfolio_optimizer.py` | Portfolio optimization math |

## Data Flow
```
Active positions + pending intents → PortfolioRiskController.evaluate()
                                          ↓
                              RiskVerdict (APPROVED/REJECTED/REDUCED/NET_OUT)
```

## Inbound Dependencies
| Module | What is imported | Why |
|--------|-----------------|-----|
| contracts/enums | RiskDecisionStatus | Verdict status |
| metrics/portfolio_optimizer | Optimization functions | Capital allocation |

## Outbound Dependents
| Module | What it imports | Why |
|--------|-----------------|-----|
| runtime/live_cycle | PortfolioRiskController | Cross-strategy risk gate |
| execution/strategy_line | RiskVerdict | Strategy-level gating |

## Known Issues

## Fix History
| Fix ID | Date | Author | Commit | Summary | Root Cause |
| FIX-20260525-025 | 2026-05-25 | cursor-agent | — | Price guard Fail-Closed: `check()` had optional `current_price` parameter — when None, gross/net exposure checks wrapped in `if current_price is not None` silently skipped, allowing blind entries without exposure validation. Fix: (1) moved concentration check (price-independent, step 0.6) BEFORE price guard (step 0.7); (2) added hard REJECTED when `current_price is None or <= 0` with reason `price_unavailable_exposure_blind`; (3) removed now-redundant `if current_price is not None` wrappers from gross/net blocks. Fail-Closed principle: no price → no exposure awareness → no entry. | RC-06 (contract-violation — price guard not enforced) |
| FIX-20260524-043 | 2026-05-24 | cursor-agent | — | T2-H1: VaR/CVaR exception logged (was bare except:pass) + equity fallback 10k→100k. T2-H2: correlation exception returns 1.0 (fully correlated) instead of 0.0 (no penalty) — data corruption no longer silently reduces risk penalty. T2-H4: added check_portfolio_stop_loss() for aggregate PnL-based LIQUIDATE_ONLY verdict. T2-H6: exposure check skipped when price unavailable (was dimensional error comparing lots vs notional %). T2-H9: VaR data insufficiency now warns conservatively (assumes risk at var_max_pct × equity) instead of returning 0.0 (false safety). | RC-06, RC-07, RC-05 |
| FIX-20260522-023 | 2026-05-22 | cursor-agent | 24ff517 | Batch mypy type safety: annotation fixes, None guards, type narrowing | type-confusion |
| FIX-20260519-008 | 2026-05-19 | cursor-agent | — | Global Directional Cooldown: 新增net_out_cooldown_seconds(默认600s)+last_net_out_timestamp/last_net_out_direction追踪。net_out强制平仓后记录被平仓方向,cooldown期间拦截该方向所有新开单(任意策略),阻断net_out→新开仓→反向net_out的死亡连锁。Cooldown检查在策略重复检查之后、总敞口检查之前执行。 | RC-12 |
| FIX-20260517-007 | 2026-05-17 | cursor-agent | — | CapitalAllocator: capacity-aware position sizing with two defense lines — max_concentration (50% default, prevents single-brain hot-streak dominance) + min_lot_size gating (prevents sub-minimum-lot micro-orders). Proportional allocation based on DynamicBrainWeighter brain weights. | missing-feature |
| FIX-20260524-042 | 2026-05-24 | cursor-agent | — | T1-H1: Symbol Quarantine — PortfolioRiskController now tracks per-symbol quarantine state (60s duration). quarantine_symbol() triggers on unconfirmed net-out close; is_symbol_quarantined() blocks all new entry orders for locked symbols. Auto-clear on independent MT5 zeros-position confirmation. check() method checks quarantine as first evaluation step. | RC-06 (double-position risk from unconfirmed close) |

## Cross-Module Contracts
| Contract | Consumers | Stability |
|----------|-----------|----------|
| `PortfolioRiskController.evaluate(intents, positions)` → `RiskResult` | live_cycle | Stable |
| `compute_optimal_group_weights(groups, constraints)` → `dict[group, weight]` | capital_allocator | Evolving |

## Verification
```bash
python -m pytest tests/ -k "portfolio or risk" -q
```
