...
| FIX-20260824-004 | 2026-08-24 | cursor-agent | (pending) | **Live Fire 敢死队 magic 注册 (FIX-20260824-004, IC 方向 B 裁决)**: `core/contracts/strategy_magic.py` `_HARDCODED_FALLBACK` 新增 `90601: "micro_scaler_v2_live_fire"` — 不注册 strategy_line (旁路派发), 独立 magic 专属 journal 归属 + 熔断器按 magic 聚合. 与 sentinel 90401 (`__UNATTRIBUTED_BRIDGE_DEFAULT__`, 必须永远不出现于真实开单) 隔离; 敢死队 open 必带 90601 → SENTINEL_UNATTRIBUTED_MAGIC 守卫天然放行. | IC 裁决 — 敢死队需独立可追溯 magic, 避免与任何 strategy_line 归属冲突 |
