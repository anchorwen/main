...
| FIX-20260605-127 | 2026-06-05 | cursor-agent | d9d9f49 | **Calibrator feed decoupled from ML pipeline**: FIX-028 hardcoded skip starved ConformalCalibrator (last sample May 28). New `_step_calibrator_feed()` in daily_ops.py reads closed trades from journal and independently feeds calibrator.update(). | RC-06 |
...
| FIX-20260612-002 | 2026-06-12 | cursor-agent | — | Positional fragility eradicated: `_build_feature_array()` replaces `list(features.values())` with name-ordered projection from adapter's brain_entry["features"] SSOT. Fixes 2 `.values()` call sites in replay buffer + partial_fit paths. | RC-06 |
