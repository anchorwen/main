...
|--------|------|--------|--------|---------|------------|
| FIX-20260806-005 | 2026-08-06 | cursor-agent | — | **测试幽灵告警泄漏实盘钉钉群根治 (DQAF-20260806-002, IC Option C — The Test Domain Isolation)**: `_alert_violation` 对 `@phantom` 违例构造 `LiveAlertHub(base_dir="data")` → env webhook 接线真实钉钉 → 测试故意违例时推送真实 CRITICAL (05:03:23 三条 phantom:test_*). 否决 Option A (生产代码测试门控 = Test-Induced Design Damage). 修复: test_phantom_contract.py autouse fixture 让 `LiveAlertHub.__init__` raise ImportError → 走既有 stderr fallback (counter 保留, hub/线程/audit-log 副作用全消) + 新 tests/conftest.py 全局 autouse delenv 四 webhook env (兜底). 零生产代码改动. 66+40+300 tests PASS, 审计日志零增长. 详见 FIX_REGISTRY. ReB: TEST_TO_PROD_ALERT_LEAK. | RC-06 — contract-violation: 测试域边界缺失, 生产告警路径对调用方零感知 |
| FIX-20260626-143 | 2026-06-26 | cursor-agent | — | **Mypy zero: adapters.py type-var + phantom_contract.py LiveAlertHub dead code**. (a) adapters.py: reapplied `# type: ignore[type-var]` — mypy 1.20.2 cache bug caused false `unused-ignore`. (b) phantom_contract.py: replaced nonexistent `LiveAlertHub.instance()` with `LiveAlertHub(base_dir="data")`, fixed `send_critical()` call signature (component/message/metadata → reason/detail) — previously dead code always fell through to stderr via AttributeError catch. | L2 — phantom_contract alert path never exercised; dead code since creation |
