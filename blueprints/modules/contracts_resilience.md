...
|--------|------|--------|--------|---------|------------|
| FIX-20260806-003 | 2026-08-06 | cursor-agent | — | **A2 mypy 清零 — `adapters.py` `bridge_result_or_raise` type-var 双模式根治**: 移除 `# type: ignore[type-var]` (孤立/统一检查判定冲突, FIX-20260626-143 同源), 改加 type-witness 可选参数 `_witness: type[T] | None = None` → T 在 `follow_imports=skip` 孤立模式下仍可见, 两模式均 mypy-clean 且向后兼容 (测试无需改). 详见 FIX_REGISTRY. | L2 — 裸 TypeVar 返回 + follow_imports=skip 孤立检查 erase 泛型 → ignore 与两模式判定冲突 |
