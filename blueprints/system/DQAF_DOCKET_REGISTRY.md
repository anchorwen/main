...
| DQAF-20260621-033 | 2026-06-21 | Sev 2 | **37.1% MIA/Breakeven异常关闭: 超过1/3仓位从未到达TP/SL即被外部关闭** — mt5_deal_reason_3(96笔)+unknown_close(4笔)+mia_close→PnL蒸发。需溯源MT5 Bridge订单状态同步延迟+Orphan Sweeper误杀正常持仓。关联历史: DQAF-20260610-001(MIA管道PnL缺失), DQAF-20260609-001(MIA/孤儿仓位放大)。 | **AWAITING_IC_APPROVAL** → 下周基建悬赏 | — |
| DQAF-20260621-034 | 2026-06-21 | Sev 2 | **Trailing SL 50.6%仓位0-1次快照=SL从未移动**: 131/259笔交易trail未激活(0%胜率,avg=0R)。Snapshot守护进程可能在开仓瞬间未挂载监听器或存在竞态条件(ticket注册晚于snapshot采集)。关联: blueprints/modules/execution_trail_stop.md。 | **AWAITING_IC_APPROVAL** → 下周锁/时序审计 | — |
