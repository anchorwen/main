...
- **Files**: core/runtime/trail_dispatch.py,core/runtime/management_phase.py
- **Description**: Pre-Trade Feasibility Gateway: breakeven SL validated against MT5 ask/bid constraints before dispatch. Dynamic spread-proportional buffer (2x spread + exit_min_step floor) prevents invalid SL sends when price crosses entry between trigger and dispatch.
...
- **Type**: fix
- **Module**: training
- **Files**: configs/brains/Barrier_V9_12B_V1.json,data/models/barrier/Barrier_V9_12B_V1_model.json,data/models/brains/Barrier_V9_12B_V1_model.json,scripts/training/train_swing_v9.py,core/execution/barrier_strategy.py,core/features/feature_assembler.py,blueprints/system/XAU_INSTITUTIONAL_STANDARDIZATION.md,blueprints/system/ROOT_CAUSE_DIAGNOSIS_PROTOCOL.md
- **Description**: Retire Barrier_V9_12B_V1: delete failed model config+2 artifacts, update all source/blueprint references to V2. V1 metrics (PF=0.89 SR=-6.54) vs V2 (PF=1.32 SR=14.11). V2 already sole barrier brain in live.yaml.
- **Root Cause**: RC-09 — config-drift
- **Prevention**: (to be filled)
- **Dependents Checked**: (none)
...
- **Files**: configs/brains_btc/BTC_Swing_V12_H1_15.json
- **Description**: Unfreeze V12_H1_15 config status: shadow→probation + vote_weight: 0.0→1.0 to stop SSOT reconciliation ping-pong
- **Root Cause**: RC-09 — config-drift
- **Prevention**: (to be filled)
- **Dependents Checked**: (none)
...
- **Type**: fix
- **Module**: deployment-lifecycle
- **Files**: configs/brains/Swing_V10_H1_Directional.json
- **Description**: Sync Swing_V10_H1_Directional artifact_hash with actual model file (model re-saved Jun 15, config hash stale from Jun 11 training)
- **Root Cause**: RC-09 — config-drift
- **Prevention**: (to be filled)
- **Dependents Checked**: (none)
...
- **Commit**: —
- **Type**: fix
- **Module**: daily_ops, governance
- **Files**: scripts/daily_ops.py
- **Description**: DQAF-062 L3: Config→Governance Reconciliation Gate — new _step_config_gov_reconcile() that runs before governance cycle. Reads all brain_registry_entry.v1 configs from resolved brains_dir, bootstrap-registers missing brains (register_brain("candidate")), logs status drift between config and governance WITHOUT overriding governance lifecycle (honoring FIX-20260613-076 contract). Integrated into run_daily_ops() main flow: reconcile → governance → reconcile_pnl. Eliminates the dual-track drift where 31/49 XAU brain configs had no corresponding governance entry, making set_performance_metrics() a silent no-op.
- **Root Cause**: RC-09, RC-12 — config-drift, missing-feature: no automated Config→Governance registration pipeline. Only ran at governance creation time (first-time _load_or_create_governance), never on subsequent cycles.
- **Prevention**: Reconciliation gate runs every daily_ops cycle. Bootstrap always registers as "candidate" (safety valve: prevents premature live from config typos). Drift log capped at 20 entries, surfaced in daily ops report.
- **Dependents Checked**: governance, deployment_lifecycle
...
- **Commit**: —
- **Type**: fix
- **Module**: governance, daily_ops, deployment
- **Files**: scripts/training/governance_scheduler.py,core/deployment/scheduler_service.py
- **Description**: DQAF-061 L3: XAU Governance Blind Eye Recovery — three-pronged fix. (1) governance_scheduler.py: auto-register brains from PnLStore/metrics sources in run_governance_cycle() before calling set_performance_metrics(). Silent skip (governance_service.py:146-148 — _brain_states.get() returns None → silently does nothing) now preceded by register_brain("candidate") for unregistered brains. (2) governance_scheduler.py: _data_source "no_data" marker for brains with sample_count==0 to prevent downstream false confidence. (3) scheduler_service.py: purge logic now respects _data_source field (live_journal/pnl_store) in addition to source field (brain_performance), preventing daily_ops-injected metrics from being wiped by MT5 scheduler purge. Fixes the silent disconnect where 49 brains had PnL data (144K settled entries) but only 18 received governance injection.
- **Root Cause**: RC-09, RC-06 — config-drift, contract-violation: (a) governance_service.set_performance_metrics() silently skipped unregistered brains, (b) scheduler_service purge checked source but not _data_source field, (c) no auto-registration path from PnL data sources into governance
- **Prevention**: Auto-registration runs every governance cycle before metrics injection. Purge checks both source markers. Registration gap detected via bootstrap_registered count in daily_ops report.
- **Dependents Checked**: governance, deployment_lifecycle, feedback_pnl
...
- **Type**: fix
- **Module**: feedback_pnl
- **Files**: core/feedback/brain_pnl_ledger.py,core/brains/services/brain_leaderboard.py,core/brains/services/dynamic_brain_weighter.py,scripts/training/governance_scheduler.py,scripts/daily_ops.py,apps/monitor/live_trading_dashboard.py,tests/engine/test_brain_pnl_ledger.py
- **Description**: DQAF-060 L3: Event Sourcing semantic conflation — load_from_stream() now filters by event_type (only SignalSettled enters _settled), PF sentinel 999.0 removed (float('inf') preserved), _data_source lineage honesty (live_journal vs pnl_store), augmented journal freshness integrated into daily_ops pipeline
- **Root Cause**: RC-03 — state-leak
- **Prevention**: (to be filled)
- **Dependents Checked**: brains_services,training,runtime_live,monitor_dashboard
...
- **Commit**: e90ee8f4
- **Type**: fix
- **Module**: features_service
- **Files**: core/features/computers/btc_feature_augmenter.py,core/runtime/management_phase.py
- **Description**: DQAF-059: BTCFeatureAugmenter NaN injection via np.asarray(None) — 3-layer institutional fix. Layer 1: None-safe array coercion in augment(). Layer 2: management_phase passes np.zeros(9) instead of None. Layer 3: assert replaced with np.nan_to_num + debounced WARNING log.
...
- **Files**: scripts/training/trainers/xgb_trainer.py,scripts/training/train_btc_swing_v9.py,core/deployment/brain_registration_gate.py,core/deployment/brain_config_validator.py,configs/brains_btc/BTC_Swing_V4.json,configs/brains_btc/BTC_Swing_V12_H1_15.json
- **Description**: L3 Architecture Fix: Restore BTC direction diversity by retraining V4+V12_H1_15 as 3-class (multi:softprob/multiclass). Fix xgb_trainer label mapping inversion. Add objective field validation + dynamic inference from model artifacts. Add training gate with direction diversity + Wasserstein distance check.
- **Root Cause**: RC-09 — config-drift
- **Prevention**: (to be filled)
- **Dependents Checked**: (none)
...
- **Type**: fix
- **Module**: training
- **Files**: scripts/training/train_exit_metamodel.py
- **Description**: train_exit_metamodel.py: 19-dim ExitFeatureSnapshot upgrade + hard assertion. Architecture Committee rectification. Hard assertion (sys.exit(1)) on 8-dim journal contamination. First training: 32 paired/7 wins/21.88% WR.
...
- **Files**: scripts/check_symbol_liveness.py
- **Description**: check_symbol_liveness.py: institutional preflight probe. Architecture Committee rectification. SYMBOL_DATA_DIR_MAP corrected: XAUUSDc→data (was data_xau). Iron Law #11 machine enforcement.
- **Root Cause**: RC-12 — missing-feature
- **Prevention**: (to be filled)
- **Dependents Checked**: (none)
...
- **Module**: deployment-config
- **Files**: .github/workflows/ci-windows.yml
- **Description**: Prevent pytest atexit cleanup hang with Windows Defender: tmp_path_retention_count=0→50. Pytest atexit deletes temp files one-by-one; Defender scans each deletion → 14+ min post-test hang. Retention=50 avoids cleanup at session end; pre-run wipe (rmdir /s /q) handles cleanup before next run.
...
- **Type**: perf
- **Module**: deployment-config
- **Files**: tests/execution/test_mt5_worker.py,tests/runtime/test_position_registration.py,tests/unit/test_exit_watchdog.py,tests/deployment/test_blue_green.py
- **Description**: Fix CI fast-track pytest timeout: eliminate 100s of unnecessary test waits. Fixed test_start_stores_terminal_path (mock _ready.set), test_skip_when_ticket_not_in_journal (mock time.sleep for 60x retry loop), test_payload_contains_required_fields (mock _poll_ack). Moved test_history_limited to slow track (25s drain sleeps unavoidable). Total savings: ~100s local → ~5min CI.
- **Root Cause**: RC-09 — config-drift
- **Prevention**: (to be filled)
- **Dependents Checked**: (none)
...
- **Module**: deployment-config
- **Files**: .github/workflows/ci-windows.yml
- **Description**: CI fast-track pytest timeout 10→20min: local 4min but CI Windows runner 2.5x slower, hitting 10min timeout on every run since CI #588
- **Root Cause**: RC-09 — config-drift
- **Prevention**: (to be filled)
- **Dependents Checked**: (none)
...
- **Description**: FIX-20260626-002: Raise _ctxlib_pf import from conditional block to module-level contextlib. Nested import inside if portfolio_risk_controller block caused UnboundLocalError at 10 downstream sites when portfolio is None (shutdown path).
- **Root Cause**: RC-02 — type-confusion
- **Prevention**: (to be filled)
- **Dependents Checked**: (none)
...
- **Type**: fix
- **Module**: runtime-live
- **Files**: core/runtime/golden_master.py,core/runtime/live_cycle.py,data_btc/regime_snapshots.jsonl,data/regime_snapshots.jsonl
- **Description**: Golden Master silent gap defense-in-depth: fixed blind except OSError:pass in golden_master.py (zero-logging), added defensive AttributeError handling for non-dict regime_info, broadened live_cycle.py exception handler to include AttributeError. Rebuilt regime_snapshots from GM+FS sources.
- **Root Cause**: RC-06 — contract-violation
- **Prevention**: (to be filled)
- **Dependents Checked**: (none)
...
- **Files**: scripts/training/brain_promotion_runner.py
- **Description**: Augment BrainPromotionEvaluator with journal-based PnL. Tracker profit_factor was composite_score proxy; override with actual MT5 trade PnL from journal. Closes data-source gap that caused high-PF brains to be undervalued by promotion evaluator. (DQAF-20260625-060 Phase 2)
- **Root Cause**: RC-12 — missing-feature
- **Prevention**: (to be filled)
- **Dependents Checked**: (none)
...
- **Module**: brains-services
- **Files**: scripts/training/brain_promotion_runner.py
- **Description**: Fix _compute_metrics_from_tracker() consecutive losses: lifetime max to tail (DQAF-20260625-060). 5 brains incorrectly flagged for freeze rescued.
- **Root Cause**: RC-06 — contract-violation
- **Prevention**: (to be filled)
- **Dependents Checked**: (none)
...
- **Description**: DQAF-134: brain_votes wired to journal — fixes 379/384 brain_votes:null entries in live_trade_journal. _make_decision() refactoring left brain_votes parameter declared but unwired at all call sites. Now built from proposals in the entry_context loop and passed to _make_decision() so downstream attribution (brain_attribution_service) can use real brain-vote data instead of _unknown_ fallback. Format: [{brain_id, direction_bias, confidence}].
- **Root Cause**: RC-06 — contract-violation
- **Prevention**: (to be filled)
- **Dependents Checked**: (none)
...
- **Files**: tests/observability/test_live_alert_hub.py
- **Description**: Flaky test: test_summary_emission — time.sleep(0.06) insufficient on Windows (~15.6ms timer quantum), raised to 0.15s to safely exceed 50ms storm_summary_interval
...
- **Type**: feat
- **Module**: runtime-live
- **Files**: scripts/diagnose_process_health.py
- **Description**: Process health diagnostic script: diagnose_process_health.py, Iron Law #11 compliant
- **Root Cause**: RC-12 — missing-feature
- **Prevention**: (to be filled)
- **Dependents Checked**: (none)
...
- **Type**: feat
- **Module**: deployment-lifecycle
- **Files**: scripts/phase4_shadow_review.py,scripts/phase4_final_audit.py,scripts/check_blueprint_compliance.py
- **Description**: Phase 4 shadow verification scripts: phase4_shadow_review.py + phase4_final_audit.py, MODULE_SOURCE_MAP update for 7 new files
- **Root Cause**: RC-12 — missing-feature
- **Prevention**: (to be filled)
- **Dependents Checked**: (none)
...
- **Files**: scripts/scan_barrier_params.py,scripts/backtest_rule_strategies.py,configs/training/barrier_12bar_lightgbm_v1.2.1.yaml,configs/training/label_contracts/label-survival-barrier-1.2.1.json
- **Description**: Barrier v1.2.1 contract: scan_barrier_params.py + backtest_rule_strategies.py + training config (SL=2.0, TP=1.25, H=12) + label contract
- **Root Cause**: RC-12 — missing-feature
- **Prevention**: (to be filled)
- **Dependents Checked**: (none)
...
- **Type**: feat
- **Module**: deployment-lifecycle
- **Files**: scripts/audit_memory.py
- **Description**: Memory audit script: detects stale/untracked/completed memory files. Classifies 39 memory files by staleness, completion, and tracking. --sweep archives completed to _archive/ and updates MEMORY.md.
- **Root Cause**: RC-12 — missing-feature
- **Prevention**: (to be filled)
- **Dependents Checked**: scripts/audit_memory.py
...
- **Re-enable Conditions** (TODO in memory, revised 2026-06-28 per Architecture Committee):
  1. ≥500 completed trades (symbol-agnostic) — BTC has 600 ✅
