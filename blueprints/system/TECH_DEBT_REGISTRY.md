...
| TECH_DEBT-021 | 2026-08-21 | L2 | scripts (data pipeline) | **XAU asof_join_rate 22.3% 过低 — readiness 度量分母缺陷 (The As-of Denominator, Phase 3 P1.5 The Temporal Alignment)**. 取证 (Iron Law #11, `_audit_asof_join_miss_20260821.py` + builder 交叉验证): **22.3% 是假象** — `check_training_readiness.py:846` `asof_join_rate = n_samples / journal_closed`, 分母 `_count_journal_closed` = **原始 `ack_status=="closed"` journal 条目 (4697)**, 含 677 条空 PnL + 无 ticket/孤儿 close + 每票重复 close; builder 真实 join 宇宙 = **去重含 PnL 交易 1262**, 实配 1046 = **82.9% ≥ 80% 门槛**. 时区撕裂/精度不对齐 **双否** (label naive 秒 / feature +00:00 微秒, `[:26]` 截断丢弃 offset 恰补 UTC; 匹配 gap p50=5s max=634s 全 ≤15min). 真残留: **211 STALE (16.7%)** = 真实 feature 断供窗 (05-18→05-23 131.9h + 周末 24h×N, 59 个 >6h 窗) 被 Iron Law #3 正确拒签; 5 NOT_KNOWN (0.4%) = 05-05 回填边界. | ✅ 待 DQAF-20260821-002 IC 裁决 — 主修: readiness `asof_join_rate`/`pnl_completeness` 分母收敛到 builder join 宇宙 (去重含 PnL 交易) 或独立暴露两个口径; 次修: feature 断供窗监控 (可选). ✅ **已清偿 FIX-20260821-007 (8/21, P1.5, IC 雷霆裁决 The Denominator Alignment)**: readiness 分母收敛 builder SSOT — builder Report JSON 边车 `valid_trades_count=1262` 权威分母 + 阶段2 去重宇宙排除 manual_close/orphan. asof_join_rate **82.9% (1046/1262) PASS** + pnl_completeness **0/1238 = 0.0% PASS** — XAU 就绪全绿. 次生: TECH_DEBT-022 独立建档 (特征断供无强告警, Sev 3, 留待排期, 绝不并案). |
...
| TECH_DEBT-022 | 2026-08-21 | L2 | observability | **特征流长时间断供无强告警机制 (The Silent Feature Drought, Sev 3 监控管道)**: TECH_DEBT-021 取证暴露 XAU feature store 真实断供窗 — **05-18T10:10→05-23T22:02 = 131.9h (5.5 天)** + 每周五 24h 周末缺口 ×6, 共 59 个 >6h 窗, STALE gap p50≈1.8 天. 当前系统对"开仓时刻无近期特征"仅由 builder asof_join 拒签 (Iron Law #3 按设计) 静默减样本, **无独立监控/强告警** 提示数据供应断供 → 断供可长期潜伏侵蚀训练数据质量而不被运维察觉. 与 TECH_DEBT-021 (度量说谎) 不同性质 = 监控缺位, 绝不并案. | ⏳ 待排期 (IC 裁决 2026-08-21: 今夜不修, 留待后续排期) — 建议: readiness/health_checks 增加 feature 断供窗 (≥6h WARN / ≥24h CRITICAL) 检测; 复用 calendar.py 周末休市豁免 (禁再造钟). 触发后按 Scene B 立项. |
| TECH_DEBT-023 | 2026-08-23 | L3 | features (data governance) | **v9_institutional_40 静默列漂移 (The Schema Mutagenesis, Sev 2 数据治理)**: 同一 `features.jsonl` 混装两种物理列族共用 `schema_name: v9_institutional_40` — 新世代 (2026-05-24→08-22, 8,572 行) 含 `*_Price_ZScore` 缺 `*_Macro_Gold_Silver_Spread`; 旧世代 (2025-11-06→05-18, 20,479 行) 反之. builder `len(values)>=40` 对两者同时通过 → 旧世代行对 canonical schema 静默 0.0 填充 (asof_join `values.get(fn,0.0)`) → 直接全量训练 = train/serve skew 硬伤. 连锁真相: Vol_ZScore 70.52% 零填充 = 旧世代 100% / 新世代 0.1% 时域分层伪象. | ⏳ **暂不修, 记录在案** (IC 裁决 2026-08-23 The Schema Mutagenesis: 同一个 Schema Name 绝对不允许拥有两套不同的物理列族; Phase 3 后续治理靶点). 临时规避已执行: P3 训练 `train_micro_scaler_v1.py` 仅用 current-gen 列族 (8,572 行 → ASOF 576 可标注样本), 旧世代零 impute/桥接. |
...
## TECH_DEBT-021 Detail — XAU asof_join_rate 22.3% 过低 (The As-of Denominator)

- **触发**: TECH_DEBT-020 清偿后, readiness 首次真实评估 (FIX-20260821-006) 暴露 `asof_join_rate 22.3% (1046/4697) < 80%` FAIL → IC 启动 Phase 3 P1.5 The Temporal Alignment, 行动令"取证先行, 严禁猜测代码".
- **取证脚本** (Iron Law #11, stdout 唯一合法证据): `scripts/_audit_asof_join_miss_20260821.py` — 逐行复刻 builder `asof_join` (load_journal_opens / load_feature_store / binary-search / Column 3 知识时间), 全量插桩分类.
- **铁证链**:
  1. **Builder 自身输出 (权威交叉验证)**: `Journal: 1695 tickets, 1262 with PnL` / `ASOF join: 1046 matched, 0 no prior feature, 211 stale (gap > 15min), 5 not-yet-known, 0 missing data` — 与取证复刻**逐字节吻合**.
  2. **真 asof_join_rate = 1046/1262 = 82.9% ≥ 80% 门槛** — 数据集**本可构建** (40 维/1046 样本/标签 39.2% 全达标).
  3. **22.3% 假象根因 — 分母缺陷**: `check_training_readiness.py:846` `asof_rate = n_samples / journal_closed`; `_count_journal_closed` (:1031) 数 `ack_status=="closed"` **原始条目 = 4697** — 其中 677 条空 PnL, 无 ticket/孤儿 close, 每票重复 close (8648 行 journal → 4697 closed 条目 → 1654 票有 close → **仅 1262 票含 PnL**). 分母放大了 3.7× → 22.3% 而非 82.9%.
  4. **假设检验**: ①时区撕裂 **否** — label `recorded_at` naive 秒精度 (19 字符), feature `event_time` `+00:00` 微秒 (32 字符), builder `[:26]` 截断丢弃 offset → naive → 补 UTC (恰好正确, 无小时级偏移; 匹配 gap p50=5s max=634s 全 ≤15min). ②精度不对齐 **否** — 匹配 gap 秒级干净, 无 ms/s 截断丢失. ③前瞻守卫过杀 **半真** — 容差 15min 对**真实断供**正确拒签, 非 22.3% 成因.
  5. **211 STALE 真残留 (16.7% join 宇宙)**: feature 连续间隔 p50=300s (正常 M5), 但 59 个 >6h 断供窗 — **05-18T10:10→05-23T22:02 = 131.9h (5.5 天真实断供)** + 每周五 24h 周末缺口 (05-23→05-24, 05-29→05-30, ... 06-19→06-20). STALE gap p50=158730s (≈1.8 天) → 真实"近期无特征"时期, Iron Law #3 拒签按设计工作.
  6. **5 NOT_KNOWN (0.4%)**: 05-04/05-05 开仓, 邻近 feature ingested 0.1-0.6 天**后**于 label (05-05 07:03 整库回填边界) → 知识时间过滤正确判 not-yet-known. 边界可忽略.
- **根因分层**: L2 逻辑缺陷 (RC-06 metric-semantics) — readiness 两个度量 (`asof_join_rate` + `pnl_completeness` 677/4697=14.4%) 分母用**原始 closed 条目数**而非 builder 实际 join 的**去重含 PnL 交易集**, 产生确定性误报 FAIL, 掩盖真实 82.9% PASS 与真实断供残留.
- **非根因 (勿重诊)**: builder asof_join 逻辑本身**零缺陷** (复刻逐字节吻合, 匹配率达标); 时区/精度无错位; XAU feature 写入侧健康 (TECH_DEBT-020 已证).
- **修复方向** (待 DQAF-20260821-002 IC 裁决, 本次**未改任何业务逻辑代码**): ① readiness `asof_join_rate`/`pnl_completeness` 分母收敛到去重含 PnL 交易口径 (与 builder 对齐) 或显式暴露"原始条目"与"交易级"双口径; ② 可选: feature 断供窗 (≥6h) 监控入 readiness (TECH_DEBT-012 同源 last-value-freeze 域), 独立数据质量债.
- **状态**: ✅ **CLOSED (FIX-20260821-007, 2026-08-21, Phase 3 P1.5)** — IC 雷霆裁决 The Denominator Alignment 批准更优解 (Single Source of Truth): ① builder 落 `*.report.json` 边车 (journal: valid_trades_count=1262/real_closed=1238/manual_close=416/orphan=7 + asof: matched=1046/stale=211/not_known=5); ② readiness asof_join_rate 直接读 report.valid_trades_count 权威分母 (report 缺失 → 本地去重含 PnL 交易数回退, 显式标注 fallback); ③ 阶段2 全度量改去重宇宙 + 排除 manual_close/orphan → pnl_completeness 0/1238 = 0.0%. 验收: 真实 XAU 就绪评估 asof_join_rate **82.9% (1046/1262) PASS** + pnl_completeness **0.0% PASS** + closed_trade_count 1661 + sample_count 1046 + 标签 39.2%; 残余 WARN = 既有 feature_quality_outliers (越 IC 授权范围). 回归锁 +3 (test_stage2_pnl_completeness_uses_real_closed_denominator / test_stage3_asof_rate_reads_report_denominator 600/700 / test_stage3_asof_rate_fallback_to_distinct_pnl 600/2), 测试文件 10/10. 取证资产: `_audit_asof_join_miss_20260821.py` + `_audit_journal_universe_20260821.py` 留工作树 (hash-lock 豁免). 三步归档: DQAF-20260821-002 CLOSED / CCT-20260821-003 / ReB `METRIC_DENOMINATOR_SEMANTIC_SHIFT`.
- **关联**: DQAF-20260821-002 (本 docket, CLOSED), FIX-20260821-007 (本清偿), TECH_DEBT-020 (触发源, CLOSED), TECH_DEBT-022 (次生监控债, 独立建档绝不并案), ReB `METRIC_DENOMINATOR_SEMANTIC_SHIFT`

## TECH_DEBT-022 Detail — 特征流长时间断供无强告警机制 (The Silent Feature Drought)

- **触发**: TECH_DEBT-021 清偿取证 (FIX-20260821-007, P1.5 The Temporal Alignment) 时, `_audit_asof_join_miss_20260821.py` 暴露 XAU feature store 真实断供窗 — **05-18T10:10→05-23T22:02 = 131.9h (5.5 天)** + 每周五 24h 周末缺口 ×6 (05-23/05-29/05-30/06-05/06-06/06-12/06-13/06-19...), 共 59 个 >6h 断供窗, STALE gap p50=158730s (≈1.8 天). 211 条 asof_join 被 Iron Law #3 拒签 (按设计) — **但系统无任何独立监控感知这次断供**.
- **问题本质** (Sev 3, 监控管道): 开仓时刻无近期特征 → builder asof_join 静默拒签减样本 → 训练数据质量缓慢侵蚀, 运维/IC 无感知. 断供是**真实事件** (05-18 系统未运行 5.5 天 / 周末市场休市), 应被监控显式捕获而非静默吸收. 与分母度量缺陷 (TECH_DEBT-021) **不同性质**: 021 是"度量说谎", 022 是"监控缺位" — **绝不并案**.
- **修复方向** (待排期, IC 裁决 2026-08-21: **今夜不修, 留待后续排期**): readiness 或 health_checks 增加 feature 连续间隔断供窗检测 (≥6h 告警 WARN / ≥24h CRITICAL); 复用 `core/market/calendar.py` 周末休市豁免 (TECH_DEBT-011/012 单时钟已 CLOSED — **禁再造第二个钟**); 触发后按 Scene B 立项.
- **状态**: ⏳ 待排期 (2026-08-21 IC 裁决建档, 今夜不修).
- **关联**: DQAF-20260821-002 (发现 docket, CLOSED), TECH_DEBT-021 (发现源, CLOSED, 绝不并案), TECH_DEBT-011/012 (日历时钟 CLOSED — 时间语义收敛 calendar.py)
...
## TECH_DEBT-023 Detail — v9_institutional_40 静默列漂移 (The Schema Mutagenesis)

- **触发**: P3 XAU Micro Scaler 战前质检 (DQAF-AUDIT-20260823-P3, `scripts/_audit_xau_micro_scaler_20260823.py` Section 4) — 全库 zero-fill 70.52% 表面异常, 深挖发现**不是随机数据病而是两套物理列族**.
- **现象**: `data/feature_store/records/symbol=XAUUSDc/timeframe=M5/features.jsonl` 29,051 行 v9 记录**全部恰好 40/44 键**, 但 union 44 键含两套互相排斥的列:
  - **current-gen (新)**: 含 `*_Price_ZScore` ×4, 无 `*_Macro_Gold_Silver_Spread` — 2026-05-24T22:01Z → 08-22, 8,572 行, **与 canonical schema `V9_INSTITUTIONAL_40_FEATURES` 对齐**
  - **old-gen (旧)**: 含 `*_Macro_Gold_Silver_Spread` ×4, 无 `*_Price_ZScore` — 2025-11-06 → 2026-05-18, 20,479 行
  - 无 mixed 日 → ~2026-05-24 特征引擎静默替换特征列, **未递增 schema_version**.
- **问题本质** (Sev 2, 数据治理): 同一 `schema_name: v9_institutional_40` 拥有两套物理列族. 消费者 (builder `asof_join` 的 `values.get(fn, 0.0)`) 对旧世代行静默填充 0.0 → **直接全量训练 = train/serve skew 硬伤 + 零填充伪装缺失**. 同类 schema 分裂前科: TECH_DEBT-004 (btc_macro_enhanced_37 维度分裂, RESOLVED).
- **连锁真相**: 全库 Vol_ZScore 70.52% 零填充 = 旧世代 100% (20,479/20,479) / 新世代 0.1% (9/8,572) — **时域分层伪象, 非随机数据病**.
- **IC 裁决 (2026-08-23, The Schema Mutagenesis)**: 🟢 定性严重数据债, **暂不修, 记录在案**, 作为 Phase 3 后续治理靶点. 硬约束: **同一个 Schema Name 绝对不允许拥有两套不同的物理列族** (特征引擎应递增 schema_version / 迁移旧列族).
- **临时规避 (已执行)**: P3 训练 `train_micro_scaler_v1.py` 仅用 current-gen 列族 (8,572 行 → ASOF 576 可标注样本), 旧世代行**零 impute/零桥接** — 宁可样本少, 不要 train/serve skew.
- **关联**: TECH_DEBT-004 (同类 schema 分裂前科), DQAF-AUDIT-20260823-P3 (本 docket), `train_micro_scaler_v1.py` (P3 训练, 规避实现), ReB 候选 `SCHEMA_MUTAGENESIS_SILENT_COLUMN_DRIFT`
