...
|--------|------|--------|---------|------------|
| FIX-20260607-145 | 2026-06-07 | ledger-services, scripts | **Journal compaction: atomic prune of old rejected entries (>30d)**: `compact_journal()` in `journal_cleanup.py` with `os.replace()` atomic swap + FileLock. Integrated into daily_ops. Dry-run confirmed: BTC 0 removed, XAU 13 old rejected >30d. Lazy-writer pattern (Option B) — no persistent file handles. | RC-11 |
