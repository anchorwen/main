...
| FIX-20260616-100 | 2026-06-16 | execution-guards, runtime-live | **ExitReason Classifier Vocabulary Completion + Restart Reentry Deadlock Fix (DQAF-20260616-001)**: (P0-P3) `exit_reason.classify()` 补全 "win"/"loss"/"breakeven"/None/auto_orphan_ 分类。 (P0-bis) `restart_state.py` `_reason` fallback 链新增 `label` 字段 — 修复前 close 条目的 `comment` 缺失时回退到 MT5 `detail.reason`("mt5_deal_reason_3")→classify()→UNKNOWN→重启后永久死锁。现在 `label`("breakeven"/"win") 优先于 MT5 端原因。Magic 90501 重启死锁已确认解除。 | RC-07 |
...
- **Type**: fix
- **Module**: execution-guards
- **Files**: core/execution/exit_reason.py, core/runtime/restart_state.py, tests/execution/test_reentry_guard.py
- **Description**: ExitReason Classifier Vocabulary Completion — 双层修复: (Layer 1: classify) P0 "win"→TP_HIT, "loss"→SL_HIT, "breakeven"→TIME_EXPIRED; P1 None防御; P3 auto_orphan_→UNKNOWN_CLOSE. (Layer 2: restart_state) `_reason` fallback 链新增 `label` 字段 — 修复前 close 条目缺 `comment` 时回退到 MT5 `detail.reason`("mt5_deal_reason_3")→classify()→UNKNOWN→重启后 ReentryState 被污染→永久死锁。修复后 `label`("breakeven"/"win") 优先于 MT5 端原因→classify() 正确识别→重入放行。Magic 90501 (structural_swing_v1) 双层死锁已确认解除。
