...
| FIX-20260610-009 | 2026-06-10 | execution-orders, features-service | **Schema resolution root-cause excision — 终结核辐射**: `SwingStrategy._run_inference()` L75 schema解析从适配器对象(source: adapter)倒置为大脑配置(source: b_info["feature_schema_id"]). Adapters(LightGBM/XGBoost)无`feature_schema`属性→getattr静默返回""→`"" or "v9_institutional"`→40维V9向量→所有非V9大脑(BTC 37-dim/XAU 35-dim)接收错误维度→dimension mismatch→raw_score=0.0兜底. 终结2周技术债循环(5次失败补丁:FIX-022/025/017/081/135). 新增致命合约检查: schema为空→ValueError(禁止静默退化). DQAF-20260610-003. | RC-06 |
