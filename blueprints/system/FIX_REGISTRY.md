...
|--------|------|--------|---------|------------|
| FIX-20260622-057 | 2026-06-22 | runtime_live, execution_orders, training, deployment, verify | **DQAF-057 Phase 1+2 — Journal Lock Unification + Label Multi-Contract Defense + Behavioral Diversity + Coverage Monitoring** — See DQAF-20260622-057 in DQAF_DOCKET_REGISTRY. **Phase 1** (commit 37cc04dd): P0-1 live_cycle.py `.locks`→`locks` namespace unification. P0-2 mt5_bridge_worker overflow merge FileLock. P0-3 daily_ops contract_path injection. **Phase 2** (commit 27ec65e): A1 resolve_brain_contracts() — 24 brain→contract mappings resolved from configs. A2 Journal PnL fallback — when close_price missing, use journal's own pnl field (100% close records have pnl). A3 Per-brain contract-aware classification — one label per participating brain with correct SL/TP contract, label_source tracing. B1 Behavioral diversity check in BrainRegistrationGate — raw_score Pearson r≥0.95 WARNING (live hit: Meta_Stage1 r=0.988). C1 label_coverage_pct metric in daily_ops. C2 verify.py contract_path regression gate. Result: BTC label coverage 40%→93.1%. | Phase 1: L3 lock namespace fragmentation, L2 missing FileLock + missing defense layer. Phase 2: L2 single-contract limitation + missing PnL fallback, L3 data saturation (V6/V7/V8 near-identical AUC). |
