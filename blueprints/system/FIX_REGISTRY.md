...
| FIX-20260610-007 | 2026-06-10 | execution-guards, brains-services, runtime-live, observability, training | **系统解冻+DataHealth基建+BTC MetaFilter V2**: (Phase 1-3) budget跨日重置+leaderboard等权兜底+calibrator持久化. (Phase 4-5) 24源全覆盖+DingTalk适配器 tail-read 117ms→12ms. (MLOps) V1 train-serve特征空间错位(模型D1_*宏观→线上M5_*V9)紧急禁用; build_btc_metafilter_v2_dataset.py ASOF PIT join 54样本0穿越; train_btc_metafilter_v2.py 极端正则化 5-Fold CV AUC=0.82; MLOps Iron Laws #1-3(字典同构启动断言). DQAF-20260610-001全量交付. | RC-03, RC-06, RC-10, RC-12 |
