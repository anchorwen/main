...
|--------|------|--------|---------|------------|
| FIX-20260623-073 | 2026-06-23 | data_infrastructure, training, runtime_live, monitor_dashboard | **DQAF-073: Unified Ticket Resolution — SSOT Extraction Path**. Created `core/data/ticket_resolver.py` — single canonical `resolve_ticket()` function handling position_ticket → detail.order → ticket in priority order. 83 files previously used 4+ different extraction patterns for the same business concept (schema drift). P0: migrated label_builder, daily_ops, and audit_data_exhaustive. P1: touch-migration on next modification. P2: full sweep. | L3 — schema drift: 4 extraction patterns for same business concept across 83 files |
