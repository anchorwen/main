...
|--------|------|--------|---------|------------|
| FIX-20260623-075 | 2026-06-23 | deployment_lifecycle, infra | **DQAF-075: Pytest CI Slow/Fast Split — Marker-Based Test Segregation**. CI `tests/engine` suite (1648 integration/E2E tests) frequently hit 300s timeout, blocking pipeline. Fix: (1) Registered `slow` marker in pyproject.toml + conftest.py. (2) Auto-mark all `tests/engine/` tests as `slow` via `pytest_collection_modifyitems`, with 7 exemptions (fast_contracts, governance_contracts, contract_guard, preflight). (3) CI split into fast track (`-m "not slow" -o "addopts="`, ~180s no coverage) and slow track (`-m "slow"`, ~200s with coverage). (4) Fast track disables coverage to hit speed target; slow track retains coverage for quality metrics. Eliminates CI timeout risk; instant root-cause attribution (engine vs logic). | RC-09 — CI timeout caused by undifferentiated test suite; slow tests not isolatable |
