...
| FIX-20260616-005 | 2026-06-17 | infrastructure, observability | **Institutional Data Integrity System (DQAF-20260616-005)**: 9-dimension audit — reconciliation, snapshots, active_position, orphan, bridge, tick, journal_integrity (duplicates+timestamps+PnL), MT5 equity, three_way_reconciliation (journal↔ledger↔snapshots with R→USD). (Fix) position_manager empty-state persistence. (Cleanup) tombstone_orphans.py (BTC 51, XAU 2801 removed). DingTalk push. Data purity: 95%+ achieved. Remaining: 46 duplicate closes + R→USD conversion refinement needs per-trade SL distances. | RC-07 |
