...
| FIX-20260616-005 | 2026-06-17 | infrastructure, observability | **Semantic-Layer Data Integrity Auditor + Cleanup (DQAF-20260616-005)**: (Audit) 6-dimension institutional checks — reconciliation, snapshots, active_position, orphan, bridge, tick precision. DingTalk push. (Fix) position_manager.save_state() now writes empty JSON instead of deleting file when zero positions. (Cleanup) tombstone_orphans.py isolates contaminated entries (BTC: 51 orphans removed, 5.7%→0%). (Exemption) snapshot-journal gap pre-2026-05-31 classified as historical exemption. | RC-07 |
