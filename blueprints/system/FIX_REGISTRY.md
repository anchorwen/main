...
| FIX-20260616-100 | 2026-06-16 | execution-guards | **ExitReason Classifier Vocabulary Completion — 退出标签分类器词汇表补全 (DQAF-20260616-001)**: `exit_reason.classify()` 无法识别 "win"/"loss"/"breakeven" 标签, 导致 `ExitReason.UNKNOWN` 降级→reentry_guard 要求 confidence≥0.70→rule-based 策略(confidence=0.50)永久死锁。修复: "win"→TP_HIT(轻冷却), "loss"→SL_HIT(严格冷却), "breakeven"→TIME_EXPIRED(中性冷却)。新增 P1 None 防御 (114处潜在崩溃) + P3 auto_orphan_→UNKNOWN_CLOSE (系统维护不触发策略惩罚)。Magic 90501 (structural_swing_v1) 已确认解除死锁。 | RC-07 |
...
- **Type**: fix
- **Module**: execution-guards
- **Files**: core/execution/exit_reason.py, tests/execution/test_reentry_guard.py
- **Description**: ExitReason Classifier Vocabulary Completion — `exit_reason.classify()` 无法识别 "win"/"loss"/"breakeven" 标签, 导致 `ExitReason.UNKNOWN` 降级→reentry_guard 要求 confidence≥0.70→rule-based 策略(confidence=0.50)永久死锁。修复按 Ω 架构师四层路线图执行: (P0) "win"→TP_HIT (轻冷却, 0.5≥0.47通过), "loss"→SL_HIT (严格冷却), "breakeven"→TIME_EXPIRED (中性冷却, 0.5≥0.45通过)。(P1) None 防御 — `classify(None)` 返回 UNKNOWN 不崩溃 (消除 114 处潜在 AttributeError)。(P3) `auto_orphan_rejected/stale`→UNKNOWN_CLOSE (系统维护, 不触发策略惩罚)。新增 5 个单元测试。Magic 90501 (structural_swing_v1) 死锁已确认解除。
- **Root Cause**: RC-07 — missing-validation: classify() 词汇表不完整, 未覆盖执行层常用退出标签
- **Prevention**: (1) 新增退出标签时必须在 classify() 中注册精确匹配模式, (2) reentry_guard 新增分类时必须有对应的 `_MAX_THRESHOLD` ceiling + TTL hard unlock (已有门禁), (3) P2 架构债: rule-based 策略的置信度豁免机制 (Deferred Architecture Fix)
- **Deferred**: P2 — 在 reentry_guard 中引入策略类型判定 (rule-based vs ML), 对 rule-based 策略使用基于时间/价格偏离的重入条件, 而非 ML 置信度阈值
