...
| FIX-20260610-007 | 2026-06-10 | execution-guards, brains-services, runtime-live | **系统解冻 — 跨日budget重置 + leaderboard等权兜底 + calibrator持久化修复**: (1) strategy_budget.py — _reset_daily()增加paused/paused_at清除 (跨午夜永冻→解除), load_state()跨日强制unpause. (2) brain_leaderboard.py — rank()增加等权兜底(vote_weights空/全零→1/N), daily_ops.py+live_daily_recap.py接入DynamicBrainWeighter. (3) conformal_calibrator.py — _save_state()合并磁盘值max(old,new)防止daily_ops实例(compute=0)覆盖live_cycle计数. DataHealthService检出的P0/P1病灶清理. | RC-03, RC-06 |
