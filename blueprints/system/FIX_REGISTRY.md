...
| FIX-20260616-003 | 2026-06-16 | execution, strategies, training | **Direction Selection Fallacy + P3 classify + P1 Tactic A Directional Balance (DQAF-20260616-003)**: (Main) RuleEngineStrategyWrapper 奇偶校验→永远 SHORT 修复为 trend_direction. (P3) classify() 新增 close_accepted→TIME_EXPIRED. (P1) build_profitable_labels.py 新增 --min-directional-ev + --max-directional-skew 方向平衡约束 — 拒绝 EV 偏向单一方向的 (SL,TP) 配置. | RC-05 |
