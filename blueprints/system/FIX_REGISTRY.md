...
| FIX-20260611-001 | 2026-06-11 | execution-orders, scripts, brains-services | **XAU Zero-Trade Unfreeze — MetaFilter routing excision + brain promotion bridge**: (P1) Swing strategies no longer use Conformal OU gate (statarb's z-score proxy → garbage p_win 0.24). MetaFilter rejection for swing strategies now falls back to rolling_wr from PnL ledger. (P0) BrainPromotionEvaluator wired into live_intent_loop (every 50 cycles). Previously only in containerized deployment path → candidate brains never promoted → governance degradation gate blocked all trades. DQAF-20260611-001. | RC-06, RC-12 |
