...
| FIX-20260610-010 | 2026-06-10 | runtime-live, execution-orders | **Phase 10 安全闸门对齐 — Fail-Closed**: Phase 10 consensus dispatch was a completely independent decision pipeline bypassing all 7 main-eval safety gates (reentry guard, budget, max_positions, trend isolation, spread, regime, portfolio risk). FIX-009 restored real brain predictions → Phase 10 activated for first time → 4 gate bypasses in 35 min → 3 concurrent positions (limit=2). Fix: main eval decisions persisted to `state._last_eval_decisions`; Phase 10 checks ALL active groups against main eval before dispatch. Missing record → Fail-Closed (block), not Fail-Open (allow). `direction=neutral` + `confidence=0.0` naturally flows into existing low-confidence skip. DQAF-20260610-003. | RC-06, RC-12 |
