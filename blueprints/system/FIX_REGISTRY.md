...
|--------|------|--------|---------|------------|
| FIX-20260622-057 | 2026-06-22 | runtime_live, execution_orders, training, deployment, verify | **DQAF-057 Phase 1+2+3a — Journal Lock Unification + Label Multi-Contract Defense + Behavioral Diversity + Coverage Monitoring + Barrier Simulation** — See DQAF-20260622-057 in DQAF_DOCKET_REGISTRY. **Phase 1** (commit 37cc04dd): P0-1 live_cycle.py `.locks`→`locks` namespace unification. P0-2 mt5_bridge_worker overflow merge FileLock. P0-3 daily_ops contract_path injection. **Phase 2** (commit 27ec65e): A1 resolve_brain_contracts() — 24 brain→contract mappings resolved. A2 Journal PnL fallback. A3 Per-brain contract-aware classification + label_source tracing. B1 Behavioral diversity check in BrainRegistrationGate. C1 label_coverage_pct metric. C2 verify.py contract_path regression gate. Result: BTC label coverage 40%→93.1%. **Phase 3a** (this commit): full forward-walking barrier simulation replaces proximity classifier in journal mode. New helpers: `_resolve_ohlc_path()`, `_load_ohlc_cached()`, `_find_bar_index()`, `_infer_symbol_from_journal_path()`. `build_trade_records()` gains `price_data_dir` param — when available, calls `contract.build_barrier_labels()` against real OHLC data. New label fields: `barrier_method`, `barrier_sl_price`, `barrier_tp_price`, `barrier_atr`, `barrier_hit_bar`. Computes theoretical PnL from barrier hit when journal lacks close_price. XAU: 429 forward_simulation records, BTC: 29. Falls back to proximity classifier when OHLC unavailable. | Phase 1: L3 lock namespace fragmentation, L2 missing FileLock + missing defense layer. Phase 2: L2 single-contract limitation + missing PnL fallback, L3 data saturation. Phase 3a: L2 — proximity heuristic approximates barrier logic with 20% tolerance; forward-walking simulation is ground truth. |
