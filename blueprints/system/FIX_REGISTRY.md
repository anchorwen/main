...
| FIX-20260616-003 | 2026-06-16 | execution, strategies | **Direction Selection Fallacy — 结构性单向开空修复 (DQAF-20260616-003)**: RuleEngineStrategyWrapper 使用 `_bars_since_last_signal % 2` 奇偶校验选方向, 但 cooldown_bars=3 导致冷却通过时计数器恒为 3→3%2=1→永远 SHORT. LONG 分支不可达. 修复: 使用已传入的 `trend_direction` (regime gate ADX-based) 替代奇偶校验. neutral 趋势时不开单. 10/10 单向开空 → 双向开仓. | RC-05 |
