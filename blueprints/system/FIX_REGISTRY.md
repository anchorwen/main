...
|--------|------|--------|---------|------------|
| FIX-20260622-057 | 2026-06-22 | runtime_live, execution_orders | **DQAF-057 Phase 1 — Journal Lock Namespace Unification + Label Contract Defense Layer**: P0-1: live_cycle.py:1338 `.locks`→`locks` (the ONLY hot-path writer still using the legacy dot-prefix lock directory — namespace fragmentation rendered FileLock cross-process serialisation ineffective, causing timestamp inversions to INCREASE from 336→365 XAU / 22→52 BTC after FIX-077). P0-2: mt5_bridge_worker.py `_merge_overflow_files` zero-lock write path now protected by FileLock. P0-3: daily_ops.py `_step_label_builder` now passes symbol-mapped label_contract (XAU→label-survival-barrier-1.0.0, BTC→label-micro-barrier-1.0.0) activating barrier-based label fallback when close_price is missing from journal (66% positions close outside system control via DEAL_REASON_SIGNAL). | P0-1: L3 — lock namespace fragmentation (architecture: multiple journal writers, inconsistent lock directories). P0-2: L2 — missing FileLock on shared journal write path. P0-3: L2 — missing defense layer: label_builder had no contract-driven SL/TP fallback when close_price absent. |
