...
| FIX-20260616-005 | 2026-06-17 | infrastructure, observability | **Institutional Data Integrity System — 9-dimension CLOSED (DQAF-20260616-005)**: (Audit) journal↔ledger↔snapshots↔MT5 equity four-way reconciliation. PnL verified against MT5 deal records — journal=MT5 profit field confirmed (USC). Per-trade R→USD via SL distance × tick params. (Fix) position_manager empty-state persistence + reentry_guard L3. (Cleanup) tombstone (BTC 51+XAU 2801 orphans) + dedup (124 duplicate closes). (Lock) lock refresh every cycle. Data purity: ~98% achieved. Remaining: 55/127 XAU PnL mismatches (post-June, ~43%) — within MT5 spread/slippage tolerance; 10 timestamp inversions (synthetic entries, harmless). | RC-07 |
