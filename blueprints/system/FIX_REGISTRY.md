...
| FIX-20260622-059b | 2026-06-22 | contracts_ids, execution_orders, runtime_live, scripts, feedback_pnl | **DQAF-059 Production Deployment Hardening — IC Approved Three-Layer Checklist**. P2 (Fail-Fast): `UnattributedOrderRejected` exception class + `SENTINEL_UNATTRIBUTED_MAGIC=90401` sentinel constant in `strategy_magic.py`; `dispatch_live_open_order()` validates magic≠90401 before building execution_payload → raises immediately; `execution_queue` catches `UnattributedOrderRejected` specifically (before generic `except Exception`), adds strategy to `_unattributed_blocked` set → permanent lock until restart — no retry loop; `strategy_evaluator.py` checks `is_unattributed_blocked()` before enqueue → blocked strategies never reach dispatch. P1 (MT5 Timeout): `_write_zmq_journal_entry` MT5 fallback upgraded from bare `positions_get`/`history_deals_get` to `mt5_call_with_timeout(3s)` using existing fault_handler infrastructure — prevents journal write path from blocking on MT5 IPC hangs. P0 (Integration Verified): `augment_journal_strategy.py` executed for BTC (74 backfilled) + XAU (45 backfilled); `live_journal_metrics.py` + `build_live_labeled_dataset.py` confirmed preferring augmented view + filtering `__UNATTRIBUTED_BRIDGE_DEFAULT__`. | L2 — missing fail-fast on sentinel magic + missing timeout guard on cold-backup MT5 reverse-lookup path |
