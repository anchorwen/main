...
- **Done (commit 2bd560c4, 2026-08-20)**: **Single Executor/SSOT 时间轴重构** (IC 方案 A 绝对批准) — intent 降级纯信号触发器 (daily_ops_trigger.json 瞬时信标, best-effort 零异常冒泡) + launcher 子进程唯一 daily_ops 执行者 + **stamp-at-completion** 废除 stamp-at-start (state 唯一写者=launcher 成功回执) + 副作用迁移 (label prune → _step_label_prune + P12 weekend gate) + XAU 超时 600→1200s. 回归锁 21. 三步归档: DQAF-20260820-002 / CCT-20260820-002 / ReB-20260820-SYNC_HEAVY_COMPUTE_IN_HEARTBEAT_ZERO_PULSE.
- **附属 Bug 收口 (commit 48a42397, 2026-08-20, FIX-20260820-003)**: 验证期发现本 FIX 自身回归面 — launcher 成功谓词 `rc==0` 硬判定 vs daily_ops 退出码契约 rc=1="完成且应用动作" → XAU 恒 rc=1 永不 stamp → 4h 重跑循环. IC 裁决 Bulletproof Predicate: 谓词改 `returncode <= 1 and "Traceback" not in (stderr or "")` (防弹背心区分崩溃). 回归锁 4 分支. 三步归档: DQAF-20260820-003 / CCT-20260820-003 / ReB-20260820-EXIT_CODE_CONTRACT_MISMATCH_IN_SSOT_STAMP.
- **决胜收口 (2026-08-20, FIX-20260820-004, JSON Payload Authentication)**: 部署后验证揭示 FIX-20260820-003 防弹背心被证伪 — daily_ops fail_open_guard 将捕获异常 traceback 例行写 stderr (last-resort handler), training_readiness 命中空/损坏 npz (check_training_readiness.py:722 EOFError) 确定性命中每条 XAU 运行 → stderr Traceback 不具崩溃特异性 → 每条完成运行误判 FAILED → **stamp 结构性无法落盘** (solo 复现 100%: 完整 report JSON + errors=0 + rc=1, stderr 含 1 Traceback). IC 雷霆裁决 JSON Payload Authentication: 成功谓词改 `returncode <= 1 AND stdout 尾部认证出完整 report JSON (schema_version 标识)` — 崩溃走不到打印点, stderr 全噪音免疫. 回归锁 7 分支 (含 IC 指定 solo 复现桩 + **rc=1+真实结构 report (daily_recap step 含 schema_version 字符串) stamp (顶层标记认证碰撞回归)** / CCT-20260820-004 / ReB-20260820-FAILURE_DETECTION_SIGNAL_AMBIGUITY. **P3 战役 (TECH_DEBT-014) 三枪全部收口: FIX-20260820-002 (主重构) + 003 (退出码) + 004 (谓词修正) — 全面终结.**
