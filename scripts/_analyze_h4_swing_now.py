#!/usr/bin/env python3
"""
H4 Swing XAU 实盘表现审计 — Iron Law #11 合规
...
用法: python scripts/_analyze_h4_swing_now.py
"""
...
def main():
    ...
    n_total = len(settled)
    print("=== H4 Swing XAU 实盘审计 ===")
    print(
        f"Journal H4 entries: open={len(opens)}, close={len(closes)}, modify_sltp={len(modifies)}"
    )
...
""")
    ...
    for days in [7, 14, 30, 60, 90]:
        ...
        if nr:
            ...
            avg_pnl = pnl_r / nr
            print(
                f"  {days:2d}d: {nr:3d}t | WR={wr_r:5.1f}% | PnL=${pnl_r:+7.2f} | PF={pf_r:.2f} | Avg=${avg_pnl:+.2f}/trade"
            )
        else:
    ...
    label_wr: dict[str, list[float]] = defaultdict(list)
    for _, c in settled:
        ...
        label_wr[lb].append(1.0 if _pnl(c) > 0 else 0.0)

    for lb in sorted(label_counts.keys(), key=lambda k: -label_counts[k]):
        cnt = label_counts[lb]
        pnl_l = label_pnls[lb]
        wr_l = sum(label_wr[lb]) / len(label_wr[lb]) * 100 if label_wr[lb] else 0
        print(f"  {lb:45s}: {cnt:3d}t | WR={wr_l:5.1f}% | PnL=${pnl_l:+7.2f}")
    ...
    print("PnL 分布")
    print(f"{'='*60}")
    all_pnls = sorted([_pnl(c) for _, c in settled])
    if all_pnls:
        print(f"  Min:  ${min(all_pnls):+.2f}")
        print(f"  P25:  ${all_pnls[len(all_pnls)//4]:+.2f}")
        print(f"  Median: ${all_pnls[len(all_pnls)//2]:+.2f}")
        print(f"  P75:  ${all_pnls[3*len(all_pnls)//4]:+.2f}")
        print(f"  Max:  ${max(all_pnls):+.2f}")

    # ── Holding duration ──
    print(f"\n{'='*60}")
    print("持仓时长 (open→close)")
    print(f"{'='*60}")
    durations: list[float] = []
    for o, c in settled:
        open_ts = _parse_ts(o.get("recorded_at", ""))
        close_ts = _parse_ts(c.get("recorded_at", ""))
        if open_ts and close_ts:
            dur_hours = (close_ts - open_ts).total_seconds() / 3600
            durations.append(dur_hours)

    if durations:
        durations.sort()
        print(f"  Count:   {len(durations)}")
        print(f"  Min:     {min(durations):.1f}h")
        print(f"  Median:  {durations[len(durations)//2]:.1f}h")
        print(f"  Mean:    {sum(durations)/len(durations):.1f}h")
        print(f"  Max:     {max(durations):.1f}h")
        # Count by bracket
        brackets = {"<1h": 0, "1-4h": 0, "4-12h": 0, "12-24h": 0, "24-72h": 0, ">72h": 0}
        for d in durations:
            if d < 1:
                brackets["<1h"] += 1
            elif d < 4:
        ...
        for bracket, cnt in brackets.items():
            ...
            print(f"    {bracket:8s}: {cnt:3d} ({pct:4.1f}%)")
    ...
    print(f"\n[analysis complete — {n_total} settled H4 trades]")
