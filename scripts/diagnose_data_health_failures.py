...
def diagnose_entry_context_vector(data_dir: str) -> dict:
    ...
    for line in jl_path.read_text(encoding="utf-8").strip().split("\n"):
        ...
        if ctx is None or (isinstance(ctx, dict) and not ctx.get("vector")):
            missing_vector += 1
            ...
            if len(missing_samples) < 5:
                missing_samples.append({
                    ...
                    "strategy": strategy,
                    "recorded_at": ts,
                    "has_entry_context": ctx is not None,
    ...
    return {
        ...
        "missing_rate": round(missing_vector / max(total_opens, 1), 4),
...
def diagnose_journal_vs_pnl_ledger(data_dir: str) -> dict:
    ...
    result = {"journal_closes": 0, "ledger_settled": 0, "delta_pct": None}
    ...
    if ledger_path.exists():
        for line in ledger_path.read_text(encoding="utf-8").strip().split("\n"):
            ...
            if e.get("event_type") == "SignalSettled" and e.get("exit_price") is not None:
                result["ledger_settled"] += 1

    total = max(result["journal_closes"], result["ledger_settled"], 1)
    result["delta_pct"] = round(abs(result["journal_closes"] - result["ledger_settled"]) / total, 4)
    return result
...
def diagnose_brain_registry_governance(data_dir: str) -> dict:
    ...
    if governance_path.exists():
        ...
        if isinstance(gov, dict):
            # Governance state might have nested structure
            gov_brains_raw = gov.get("brains", gov.get("brain_statuses", {}))
...
if __name__ == "__main__":
    for label, d in [("BTC", "data_btc"), ("XAU", "data")]:
        ...
        print(f"  Missing vector: {r1.get('missing_vector', '?')}")
        print(f"  Missing rate: {r1.get('missing_rate', '?')}")
