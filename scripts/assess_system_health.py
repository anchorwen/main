...
new_modules = ['management_phase.py', 'trail_dispatch.py', 'modify_trail_dispatch.py',
               'mia_close.py', 'ou_hurst.py', 'cooldown.py', 'pre_close_check.py',
               'timeframe_scaling.py', 'strategy_config_validator.py', 'position_ownership.py',
               'trend_volume_guard.py', 'net_out_close_handler.py']
...
if cov_path.exists():
    ...
    total_stmts = sum(d.get('summary', {}).get('num_statements', 0) for d in cov.get('files', {}).values())
    total_covered = sum(d.get('summary', {}).get('covered_lines', 0) for d in cov.get('files', {}).values())
    ...
    try:
        ...
        for line in content.splitlines():
            if re.match(r'\s*except\s+Exception', line):
                ...
                elif 'BLE001:FOG' in line or 'fail_open_guard' in line or 'log_and_continue' in line:
                    ble001_stats['FOG'] += 1
...
large_files = ['core/runtime/live_cycle.py', 'core/runtime/management_phase.py',
               'scripts/live_intent_loop.py', 'core/execution/strategy_line.py']

for fp in large_files:
    if Path(fp).exists():
        content = Path(fp).read_text(encoding='utf-8')
        imports = [l for l in content.splitlines()
...
iron_laws = Path('CLAUDE.md').read_text(encoding='utf-8').count('Iron Law #') if Path('CLAUDE.md').exists() else 0
