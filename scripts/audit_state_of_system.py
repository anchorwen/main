        ...
        if path.exists():
            ...
            try:
                ...
                if "registry" in af:
                    ...
                    entry["record_count"] = len(records) if isinstance(records, list) else len(records) if isinstance(records, dict) else 0
                    if isinstance(records, list):
                        entry["alpha_ids"] = [
                            r.get("alpha_id", r.get("id", "?")) for r in records
                        ]
...
def decision_audit(data_dir: str) -> dict[str, Any]:
    ...
    base = PROJECT_ROOT / data_dir
    result: dict[str, Any] = {"directory": data_dir, "total_files": 0, "total_empty": 0, "total_nonempty": 0}
...
def summary_table(audits: dict[str, Any]) -> str:
    ...
    for label, data_dir in [("BTC", "data_btc"), ("XAU", "data")]:
        ...
        lines.append(f"  LEADERBOARD  entries: {lb.get('entry_count', 'N/A')}  |  total_brains: {lb.get('total_brains', 'N/A')}")

        # Alpha
        aa = fa["files"].get("reports/alpha_allocation.json", {})
        lines.append(f"  ALPHA_ALLOC  recs: {aa.get('recommendation_count', 'N/A')}  |  notional: {aa.get('total_notional', 'N/A')}  |  ids: {aa.get('alpha_ids', 'N/A')}")

        # Alpha registry
        ar = fa["files"].get("./alpha_registry.json", {})
        lines.append(f"  ALPHA_REG    records: {ar.get('record_count', 'N/A')}  |  ids: {ar.get('alpha_ids', 'N/A')}")
        ...
        # Governance
        lines.append(f"  GOVERNANCE   total: {ga.get('total_brains', 'N/A')}  |  status: {ga.get('status_distribution', {})}")
        lines.append(f"               live: {ga.get('live_brains', [])}")
        lines.append(f"               candidate: {ga.get('candidate_brains', [])}")

        # Decisions
        lines.append(f"  DECISIONS    files: {da.get('total_files', 0)}  |  empty: {da.get('total_empty', 0)}  |  nonempty: {da.get('total_nonempty', 0)}")
    ...
    lines.append(f"  Leaderboard entries:  BTC={btc_lb.get('entry_count','?')}  vs  XAU={xau_lb.get('entry_count','?')}")

    btc_gov = audits["governance_data_btc"]
    xau_gov = audits["governance_data"]
    lines.append(f"  Governance live:      BTC={btc_gov.get('live_brains',[])}  vs  XAU={xau_gov.get('live_brains',[])}")

    btc_dec = audits["decisions_data_btc"]
    xau_dec = audits["decisions_data"]
    lines.append(f"  Decision files:       BTC={btc_dec.get('total_files',0)} ({btc_dec.get('total_nonempty',0)} nonempty)  vs  XAU={xau_dec.get('total_files',0)} ({xau_dec.get('total_nonempty',0)} nonempty)")
    ...
    lines.append(f"  Alpha alloc IDs:      BTC={btc_aa.get('alpha_ids','?')}  vs  XAU={xau_aa.get('alpha_ids','?')}")
