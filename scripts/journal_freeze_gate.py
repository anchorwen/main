...
Environment bypass (for approved exceptions):
    set JOURNAL_FREEZE_BYPASS=APPROVED_BY_ARCH_REVIEW

When coverage infrastructure is ready (pytest-cov configured + coverage
reports generated), replace the env-var check with a real coverage read.
"""
...
# ── Bypass token ──
_BYPASS_TOKEN = os.environ.get("JOURNAL_FREEZE_BYPASS", "")
...
def main() -> int:
    ...
    # ── Bypass for approved exceptions ──
    if _BYPASS_TOKEN == "APPROVED_BY_ARCH_REVIEW":
        print(
            "[IRON LAW] Journal freeze bypassed by ARCH_REVIEW approval. "
            "Please document the reason in your commit message."
        )
        return 0

    # ── Coverage-based gate (active when infrastructure is ready) ──
    coverage_pct = _read_coverage_pct()
    if coverage_pct >= 80.0:
        print(
            f"✅ [铁律] 账本测试覆盖率已达标 ({coverage_pct:.1f}%), "
            "允许修改核心账本接口。"
        )
        return 0
    ...
    print("   For approved exceptions (architecture review required):")
    print("     set JOURNAL_FREEZE_BYPASS=APPROVED_BY_ARCH_REVIEW && git commit ...")
    print("=" * 72)
    return 1
