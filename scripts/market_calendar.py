...
def evaluate_pre_close(
    *, now_utc: datetime, symbol: str | None, config: dict[str, Any]
    ...
    # ── Check fixed blackouts for holiday pre-close ──
    for window in config.get("fixed_blackouts") or []:
        try:
            start = _parse_iso_z(str(window["start"]))
        except (KeyError, ValueError):
    ...
    # Also check if we're in the window AFTER a close (e.g., still open but we missed it)
    prev_close_dt = close_dt - timedelta(days=7)
    if prev_close_dt < now_utc < close_dt:
        # We are BETWEEN two closes — compute pre-close from the NEXT close
        pass
