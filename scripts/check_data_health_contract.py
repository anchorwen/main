...
def probe_alpha_registry(domain: dict, data_dir: str) -> dict[str, Any]:
    ...
    if any_wr:
        results.append(
            {"check": "win_rate_valid", "verdict": Severity.PASS, "detail": "Win rate data present"}
        )
    else:
        results.append(
            {
                "check": "win_rate_valid",
                "verdict": Severity.FATAL,
...
def probe_brain_performance(domain: dict, data_dir: str) -> dict[str, Any]:
    ...
    if zero_signal:
        results.append(
            {
                "check": "active_brains_have_signals",
                "verdict": Severity.WARN,
...
def main() -> int:
    ...
    for domain_name, domain_cfg in domains.items():
        ...
        icon = (
            ...
            if verdict == Severity.FATAL
                ...
                else (_green("[PASS]") if verdict == Severity.PASS else "[FAILED]")
            )
        )
        print(f"\n  {icon} {domain_name}")
        for r in result.get("results", []):
            sub_icon = (
                _red("  FAIL")
                if r["verdict"] in (Severity.FATAL,)
                else (_yellow("  WARN") if r["verdict"] == Severity.WARN else "  PASS")
            )
    ...
    print(f"\n{'=' * 60}")
    print(
        f"  OVERALL: {_red('FATAL') if overall == Severity.FATAL else _yellow('WARN') if overall == Severity.WARN else _green('PASS')}"
    )
    print(
        f"  FATAL: {fatal_count}, WARN: {warn_count}, FAILED: {failed_count}, PASS: {len(probes) - fatal_count - warn_count - failed_count}"
    )
    print(f"{'=' * 60}")
