...
def check(condition: bool, msg: str) -> bool:
    ...
    if condition:
        ...
        FAIL += 1; print(f"  ❌ {msg}"); return False

def warn(condition: bool, msg: str) -> bool:
    global WARN
    if not condition: WARN += 1; print(f"  ⚠️  {msg}")
    return condition


def audit_journal(path: str, label: str) -> dict:
    if not os.path.exists(path):
        print(f"\n  {label} journal: MISSING"); return {}
    ...
    for mid, entries_list in [ (mid, [e for e in entries if e.get('message_id')==mid]) for mid in dupes]:
        tickets = set(e.get('position_ticket') for e in entries_list)
        if len(tickets) > 1: cross_ticket_dupes += 1
    ...
    ts_list = [(i, str(e.get('recorded_at',''))) for i,e in enumerate(entries) if e.get('recorded_at')]
    ...
    check(len(dupes) == 0, f"{len(dupes)} duplicate message_ids (cross-ticket: {cross_ticket_dupes})")
    check(cross_ticket_dupes == 0, f"{cross_ticket_dupes} cross-ticket message_id reuse")
    warn(pnl_null <= len(closes)*0.2, f"PnL null: {pnl_null}/{len(closes)} ({pnl_null/max(len(closes),1)*100:.0f}%)")
    ...
    return {'total':total, 'opens':len(opens), 'closes':len(closes), 'tickets':len(tickets), 'dupes':len(dupes), 'cross_dupes':cross_ticket_dupes}


def audit_pnl_ledger(path: str, label: str) -> dict:
    if not os.path.exists(path):
        print(f"\n  {label} PnP Ledger: MISSING"); return {}
    with open(path, encoding='utf-8') as f:
        data = json.load(f)
    ...
    for bid, entries in settled.items():
        for e in entries:
            ...
            if ep > 0 and abs(ep - cp) < 0.01: phantom += 1
    ...
    check(identity_leaks == 0, f"Identity leaks: {identity_leaks} brain pairs with identical records")
    return {'pending':len(pending), 'settled':total_settled, 'phantom_pct':phantom_pct, 'leaks':identity_leaks}


def audit_file(path: str, label: str, min_age_min: float = 60) -> bool:
    if not os.path.exists(path):
        print(f"  {label}: MISSING"); return False
    age = (time.time() - os.path.getmtime(path)) / 60
    fresh = age < min_age_min
    if not fresh: print(f"  ⚠️  {label}: {age:.0f}min old (limit: {min_age_min}min)")
    else: print(f"  ✅ {label}: {age:.0f}min")
    return fresh
...
for symbol, data_dir, csv_label in [("BTC", "data_btc", "BTC"), ("XAU", "data", "XAU")]:
    ...
    if os.path.exists(gov_path) and os.path.exists(pnl_path):
        with open(gov_path) as f: gov = json.load(f)
        with open(pnl_path) as f: pnl = json.load(f)
